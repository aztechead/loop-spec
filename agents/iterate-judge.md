...
Use the OKF 0.2 Iteration Report header in `skills/shared/okf-artifacts.md` when writing ITERATION.md.
...
4. **Score each goal criterion 1–10, brutally honest.** Derive criteria from the original goal, not only the SPEC checkboxes. When the goal names speed, memory, or scale, one criterion is the performance stance's deliverable (`skills/shared/engineering-stances.md`): a measurement before and after, on the input the deployment grows, not an adjective. When the goal is a refactor, one criterion is that behavior did not change (the pre-existing tests pass unchanged). For every criterion below 8, write one concrete sentence on what is still weak and why.
...
- **Plain language (readability contract — advisory).** Write `weak_point`, `description`, `fix_first`, and `summary` as one plain, active-voice sentence each — no stock phrases, no hedging padding. Full reference: `skills/shared/plain-language.md`. Advisory only (`lib/plain-language-lint.sh` never blocks); it is not a gate on your verdict.
- **Confirmation mode** (`mode=confirmation` in your dispatch prompt): the iteration limit is spent and the orchestrator will ship regardless of your verdict — you cannot trigger a rewind. Judge exactly as strictly as always; your verdict only decides whether the ship is recorded as a confirmed converge or ships with your named gaps in `warnings[]`. Same JSON format.
