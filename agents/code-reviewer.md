...
8.7. **Performance pass** (canonical reference `skills/shared/engineering-stances.md`, Performance). Step 7's `scale:` asks whether work grows unbounded; this pass asks whether the work already there is needed. Read the diff as an engineer whose goals are speed, memory use, and scalability, and report each as **Important** with file:line, one line per finding: `perf:` a bottleneck (a hot path that does more work than its output needs), inefficient logic (a repeated lookup, a copy where a reference serves, an O(n^2) walk over a list the deployment grows), or unnecessary rendering (a recomputation or re-render whose inputs did not change). Name the input that makes it cost and what to measure; a finding you cannot pin to an input is Minor. When the SPEC froze behavior (a refactor), also confirm the diff adds no behavior and the pre-existing tests are unchanged; a changed assertion in a refactor is **Critical**. This pass lists; it never rewrites.
9. Classify remaining findings:
   ...
   - **Important**: bugs, perf regressions, missed test coverage, brittle code, over-engineering findings from step 6, design-for-change findings from step 7, the measurable code-for-humans findings from step 8 (`house:`, `noise:`, `churn:`, `silent:`), the evidenced docs findings from step 8.5 (`doc:`, `stale-doc:`), and the input-pinned `perf:` findings from step 8.7
