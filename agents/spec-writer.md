...
- **Build-from-scratch stance (greenfield).** Write the spec of a production-ready application's true MVP: the stack and deployment target under `## Constraints`, the walking skeleton and the interface the user meets under `## User-facing behavior`, and the scaling input the design must hold against as a Good Enough criterion. Nothing speculative, nothing that has to be thrown away. Full reference: `skills/shared/engineering-stances.md`.
