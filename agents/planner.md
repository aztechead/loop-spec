...
- **Current docs over model memory for dependency idioms.** Every dependency `lib/doc-deps.sh scan` names on your task files needs a doc-backed `EVID-NNN` or an `ASSUMPTION` bullet in `## Grounding` — `lib/doc-deps.sh gate` blocks the plan commit otherwise. Fetch the dependency's current documentation with any web search or URL-fetch tool your session provides and append the finding yourself with `bash lib/evidence.sh add <evidence_path> "<idiom question>" "<query or URL>" "<one-line finding>"` (the one write your Bash is for; the ledger lives inside your write scope). No web-capable tool: write the `ASSUMPTION` with a verify command, or return `NEEDS_CONTEXT`.
...
- **Design for scale before code exists (on by default).** Every task names its scaling input (rows, files, events, concurrent callers) and the bound the implementer must hold, in its Steps; a task with a fixed-size input says `scale: none (fixed-size input)`. A bound discovered after the test passes is a rewrite; a bound named in the plan is a constraint.
- **Stances (on by default).** Under greenfield, hold the build-from-scratch and system-design stances: PLAN.md carries a `## System design` section with one bullet each for architecture, component structure, data flows, API design, database schema, interface architecture, and caching strategy, and the `## File map` is the whole file structure. Any feature whose spec adds a component, store, service boundary, or cache fills the bullets that changed and writes `- none` with a reason for the rest. When the spec says features stay unchanged (a refactor), every task carries a behavior-preserving verify command (the tests that pass before must pass after) and one refactoring strategy from PATTERNS.md's problem areas; never a task that adds behavior. Full reference: `skills/shared/engineering-stances.md`.
- **Versions come from a tool, never from recall.** Where a task touches a stack choice, its Steps name the command the implementer runs to learn the current safe version (`npm view`, `pip index versions`, `cargo search`, `go list -m -versions`, `gem info`, the advisory check, or a registry/web tool the harness offers); a version you would write from memory is an `ASSUMPTION: ... | verify: <command>` line. Full reference: `skills/shared/engineering-directives.md`.
