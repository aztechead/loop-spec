...
- **When you would run any `claude -p` that drives the plugin end to end**, don't unless the user asked for a live run in this session. It spends real money per task, and nothing in the tree scripts one: the shipped tests are offline by policy, and findings from a live run stay out of the tree (a PR body and memory, not a document).
- **When a change makes `bash tests/run-all.sh` slower**, the ceiling is 157 s of wall clock (8 wide, network denied, set 2026-09-11 when the runner printed it); the runner prints its wall clock at the end. A run past it is a finding to fix in the same change, never a budget to raise. The shipped tests are the ones that pass with the machine offline; a test that needs the network is deleted, not replaced.
