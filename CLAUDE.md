...
- **Lean deps.** No npm/pip/brew package managers for shipped code. Runtime requires `bash >= 4`, `git`, `jq >= 1.5`, `python3 >= 3.6` (all ship on stock macOS; Alpine/distroless Linux needs `apk add jq python3`). All shipped code is markdown + bash + jq + python3 inline. One scoped exception: `extensions/pi/loop-spec.ts` is TypeScript because pi loads extensions natively — it must import node builtins ONLY (no npm install, ever; `tests/pi-extension.test.sh` enforces this).
- **Dual harness.** loop-spec ships as a Claude Code plugin AND a pi (pi.dev) package from one source tree. `lib/harness.sh` is the detection seam; `skills/shared/pi-harness.md` is the adaptation contract. Every pi accommodation must be an additive branch keyed on that probe — never a change to the Claude Code path. Keep `package.json` (pi manifest) in version lockstep with `.claude-plugin/plugin.json` (`tests/validate-pi-manifest.test.sh` enforces it), and pin new cross-file pi couplings in `tests/pi-harness-coverage.test.sh`.
