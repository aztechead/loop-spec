...
- **When a phase needs to know where something lives**, Glob/Grep/read the tree (fan out to subagents on a large tree). Cite `file:line`. Do not add a code graph, symbol index, embedding store, or generated codebase map: stored maps rot, and a rotted map is worse than none because it is wrong with authority (graphify removed in 2.35; the prose codebase mappers followed on the same evidence). Durable knowledge the code cannot express goes in human-written docs the diff that changes the behavior keeps true.
...
- **When you would tell a dispatched agent to "think like a senior engineer"**, don't write it into the prompt. Add or extend a row in `skills/shared/engineering-stances.md` (the mindset, its deliverables, and the artifact section that proves it), bind it by cite from the phase, and pin the cite in `tests/engineering-stances-coverage.test.sh`. A stance never selects a route or a phase.
...
- **When you write or edit those same files**, match the neighbors (house style over habit); comments carry why, never what; fail loudly (the failure path). Read `skills/shared/human-code.md`. Before you finish, run `bash lib/house-style.sh compare <files>`, `bash lib/comment-tells.sh scan <files>`, `bash lib/failure-tells.sh scan <files>`, `bash lib/portability-scan.sh scan <files>`. Enforced by `tests/human-code-coverage.test.sh`.
- **When you write or edit shell or Python under `lib/`, `hooks/`, `tests/`, or `extensions/`**, never assume GNU or Linux: the plugin runs on any *nix, including macOS's bash 3.2 and BSD userland. Run `bash lib/portability-scan.sh scan <files>` in the same turn; it names, with `file:line`, a bash->=4-only construct (`mapfile`, `declare -A`, `${var,,}`, `|&`, `coproc`, ...) or a GNU-coreutils-only flag (`readlink -f`, `sed -i` with no suffix, `stat -c`, `date -d`, `grep -P`, `sort -V`, `cp --parents`, `find -printf`, an `mktemp` template with fewer than 3 trailing X's) that the other platform does not accept. Fix the finding or mark the line `# portability: <reason>` when the construct is deliberate and already guarded (a `command -v` check, an `||` fallback to the other platform's flag). Pinned by `tests/lib/portability-scan.test.sh`.
...
Skills must use `${LOOP_SPEC_SKILL_DIR}` (the documented skill substitution) to reach bundled scripts, NOT `${CLAUDE_PLUGIN_ROOT}` (a hooks/MCP variable that is empty in skill Bash). A skill at `skills/<name>/` reaches `lib/` and `hooks/` via `${LOOP_SPEC_SKILL_DIR}/../../lib/...` and `${LOOP_SPEC_SKILL_DIR}/../../hooks/...`. `${CLAUDE_PLUGIN_ROOT}` remains correct in `hooks/hooks.json` and MCP/LSP configs only.
