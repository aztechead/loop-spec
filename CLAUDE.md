# super-spec Contributor Guidelines

## Philosophy

- **Lean deps.** No npm/pip/brew package managers. Runtime requires `bash >= 4`, `git`, `jq >= 1.5`, `python3 >= 3.6` (all ship on stock macOS; Alpine/distroless Linux needs `apk add jq python3`). All shipped code is markdown + bash + jq + python3 inline.
...
1. New file `agents/{role}.md` (bare role name, no `super-spec-` prefix). The harness namespaces it as `super-spec:{role}`; reference it from skills as `subagent_type: "super-spec:{role}"`.
