...
- **When you would run `evals/run.sh`, or any `claude -p` that drives the plugin end to end**, don't unless the user asked for a live eval in this session. It spends real money per task. `tests/run-all.sh` never registers it. Records land in `evals/results/`, which is ignored; the analysis of the runs so far is `evals/findings-2026-09-06.md`. Read `evals/README.md` before proposing a run.
