...
check "bash -c executable cycle runner is denied" 2 "$(bash_cmd "bash -c 'python3 extensions/sessions/cycle_run.py'")" CLAUDE_PROJECT_DIR="$ROOT"
check "python -c runpy cycle runner is denied" 2 "$(bash_cmd $'python3 -c \'import runpy; runpy.run_path("extensions/sessions/cycle_run.py")\'')" CLAUDE_PROJECT_DIR="$ROOT"
check "python -c os.system launcher is denied" 2 "$(bash_cmd $'python3 -c \'import os; os.system("claude -p /loop-spec:cycle")\'')" CLAUDE_PROJECT_DIR="$ROOT"
check "python -c subprocess launcher is denied" 2 "$(bash_cmd $'python3 -c \'import subprocess; subprocess.run(["claude", "-p", "/loop-spec:cycle"])\'')" CLAUDE_PROJECT_DIR="$ROOT"
check "python -c subprocess shell launcher is denied" 2 "$(bash_cmd $'python3 -c \'import subprocess; subprocess.run(["bash", "-c", "claude -p /loop-spec:cycle"])\'')" CLAUDE_PROJECT_DIR="$ROOT"
check "python -c subprocess echo data is allowed" 0 "$(bash_cmd $'python3 -c \'import subprocess; subprocess.run(["echo", "claude -p"])\'')" CLAUDE_PROJECT_DIR="$ROOT"
check "bash -c eval launcher is denied" 2 "$(bash_cmd $'bash -c \'eval "codex exec /loop-spec:cycle"\'')" CLAUDE_PROJECT_DIR="$ROOT"
check "bash -lc launcher is denied" 2 "$(bash_cmd "bash -lc 'claude -p /loop-spec:cycle'")" CLAUDE_PROJECT_DIR="$ROOT"
check "python -c prompt mentioning launcher is allowed" 0 "$(bash_cmd "python3 -c 'print(\\\"claude -p\\\")'")" CLAUDE_PROJECT_DIR="$ROOT"
check "python -c data string mentioning launcher is allowed" 0 "$(bash_cmd "python3 -c 'value=\\\"claude -p\\\"'")" CLAUDE_PROJECT_DIR="$ROOT"
check "echo prompt mentioning launcher is allowed" 0 "$(bash_cmd "echo 'claude -p'")" CLAUDE_PROJECT_DIR="$ROOT"
check "echo -c data mentioning launcher is allowed" 0 "$(bash_cmd $'echo -c \'os.system("claude -p /loop-spec:cycle")\'')" CLAUDE_PROJECT_DIR="$ROOT"
check "session runner prompt data mentioning launcher is allowed" 0 "$(bash_cmd "python3 extensions/sessions/session_run.py --prompt 'claude -p'")" CLAUDE_PROJECT_DIR="$ROOT"
check "session runner prompt-file argument is allowed" 0 "$(bash_cmd "python3 extensions/sessions/session_run.py --prompt-file prompt.md")" CLAUDE_PROJECT_DIR="$ROOT"
check "session runner data after separator is allowed" 0 "$(bash_cmd $'python3 extensions/sessions/session_run.py -- -c \'os.system("claude -p")\'')" CLAUDE_PROJECT_DIR="$ROOT"
