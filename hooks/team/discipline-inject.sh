...
# Reads .loop-spec/discipline.conf from CLAUDE_PROJECT_DIR (or CWD).
# Outputs a hookSpecificOutput JSON blob when ENABLED=1 in conf file.
#
# Environment variables:
#   LOOP_SPEC_DISCIPLINE   Set to "0" to disable (kill switch). Default: 1.
...
# Kill switch.
if [[ "${LOOP_SPEC_DISCIPLINE:-1}" == "0" ]]; then
  printf '{}\n'
  exit 0
fi
...
These gates are active for this session. Setting LOOP_SPEC_DISCIPLINE=0 disables injection at the hook level.'
