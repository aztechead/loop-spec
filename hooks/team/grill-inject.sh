...
# the operator's word) is a cycle or loop-runner session: nobody to grill, and every
...
Skip the grill pass **only** when the request is already unambiguous (goal, scope boundary, constraints, and acceptance are all stated), purely informational, trivially reversible, or explicitly invokes autonomous routing (`/loop-spec:auto`, `/loop-spec/auto`, `loop-spec-auto`, or an `autonomous` token). `/loop-spec:cycle` without that token is not a skip. Inside the loop-spec cycle, the SPEC interview continues the grill and the SPEC design-lock step still runs its design-shape grill; do not repeat the session-start 2-4 questions once SPEC is running.
