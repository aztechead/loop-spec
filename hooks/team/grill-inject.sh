...
Skip the grill pass when the request is already unambiguous, purely informational, trivially reversible, or explicitly invokes autonomous routing (`/loop-spec:auto`, `/loop-spec/auto`, `loop-spec-auto`, or an `autonomous` token). Inside the loop-spec cycle, the SPEC phase Socratic interview is the in-cycle realization of this directive; do not double-grill once SPEC is running.
