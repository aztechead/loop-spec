...
- `/loop-spec:status` -- read-only run dashboard: per-feature status (phase, iterations, last event, warnings, result, PR URL) and `stats` aggregates across runs (convergence rate, gate-round counts, iterate gap histogram, dispatch counts by model/role/rung, loop-fleet cost). `metrics` emits the stable, schema-versioned **metrics contract** (ROADMAP-3.0 B3) computed from the committed run digests — convergence/first-pass rates and the trailing converged streak that `lib/trust.sh` consumes; signals whose producers have not shipped yet are present as `null` so consumers fail closed. `--json` for machine consumers. Mechanics in `lib/status.sh`.
- `/loop-spec:sentinel` -- the self-sourcing seam (ROADMAP-3.0 Pillar A): `scan` runs the work-source adapters (`lib/sentinel-sources.sh`: open `loop-spec`-labeled GitHub issues, most-recent CI failure per workflow on the default branch, unchecked backlog entries, fresh assessment findings) through the **deterministic** triage policy (`lib/sentinel-triage.sh`: source weight × kind × recency, no LLM scoring, `.loop-spec/sentinel.conf` for weights/enables/queue depth) and writes the ordered queue to `.loop-spec/sentinel-queue.json`. Read-only observation in this release — nothing is executed, and items the policy cannot classify are queued as `needs-human`, never silently dropped. The offline sources (backlog, assessment) keep it useful without GitHub. `lib/trust.sh level` reports the repo's earned-autonomy level (L0–L3) from the same committed telemetry, fail-closed: with today's signals every repo computes to L0 (PR-and-wait); the level only reports — nothing consults it for authority yet.
