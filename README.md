...
- `/loop-spec:micro` -- the **micro-cycle** for small, ad-hoc tasks: the cycle's five invariants (stated done-criteria, grounded claims, test-first, evidence-before-done, mistakes-become-rules) applied inline on the main thread with zero agent ceremony. `/loop-spec:micro <task>` runs the protocol directly; `on`/`off`/`status` toggles micro mode (persists in `.loop-spec/micro.conf`, **on by default**). Micro mode is two hooks: `hooks/team/micro-inject.sh` makes the protocol ambient at SessionStart (`LOOP_SPEC_MICRO=0` kills it), and `hooks/team/adhoc-verify-guard.sh` blocks a Stop when the session edited code but no verification command ran after the last edit (`LOOP_SPEC_MICRO_GUARD=0` kills it; stands down automatically while a cycle feature is in flight, and for prose-only edits). Each task appends a ~5-line audit entry to `.loop-spec/adhoc-ledger.md` via `lib/adhoc-ledger.sh` (title, criteria, the verification command actually run, result). When a task outgrows ad-hoc scale it promotes losslessly to `/loop-spec:intake`.
