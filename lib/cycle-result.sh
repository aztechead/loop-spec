...
#   "checkpointPrHead": "<feature.json .checkpointPrHead // null: the checkpoint PR's head
#                        branch, which is <branch>-checkpoint when LOOP_SPEC_ARTIFACTS_IN_PR=0>",
  ...
  write-terminal)
    ...
    result_json="$(jq -cn --arg cycleType "$cycle_type" --arg status "$status" \
      ...
      {schema:1,loopSpecVersion:$loopSpecVersion,
       ...
       checkpointPrUrl:(if $checkpointPr == "" then null else $checkpointPr end),
       checkpointPrHead:null,delivery:null,
       converged:$converged,
  ...
  write)
    ...
    result_json="$(jq -cn \
        ...
        )) as $workDelivered |
      {
         ...
         checkpointPrHead: (if $intentionalNoChange then null else ($fj.checkpointPrHead // null) end),
