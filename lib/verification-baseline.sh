...
# The cycle's own artifacts under docs/loop-spec/ and its state under .loop-spec/ never
# change what test, lint, or typecheck see; VERIFY died on a VERIFICATION.md the lead
# had rewritten in ITERATE and not committed (6.3.0 fastapi runs), so they do not count
# as candidate dirt.
read_worktree_status() {
  git -C "$root" status --porcelain --untracked-files=all -- . ':(exclude)docs/loop-spec' ':(exclude).loop-spec' 2>/dev/null
}
