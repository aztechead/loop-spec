...
const results = await pipeline(
  criteria,
  c => agent(
    `Verify acceptance criterion ${c.id}: "${c.description}". Read ${args.specPath} and ${args.planPath}. Apply skills/shared/verification-grounding.md first: from repository ${args.repositoryRoot}, inspect git diff ${args.baseSha}..HEAD, re-read every changed file and the nearest caller/test/config/interface/contract, and populate repositoryEvidence with exactly "implementation: <repo-relative-file:line> - <what it proves>" and "integration: <repo-relative-file:line> - <what it proves>" entries. If no separate integration site exists, use "integration: none - <concrete reason>". Missing or mismatched repository evidence is FAIL. Derive the expected observables from SPEC behavior and binding decisions independently of PLAN. Inspect the proposed command assertions; strengthen focused probes if they only establish presence or omit a required part. For generated/structured contracts inspect the actual consumer-facing representation and assert required fields, types, constraints, and relationships, resolving references when present. Follow the shared validation gate and record expected versus observed evidence. Then run an adequate criterion probe, starting from the verify command \`${c.verifyCommand}\` via Bash from ${args.repositoryRoot}. Determine verdict (PASS/FAIL), capture command evidence, and set testSuiteStatus based on whether any pytest/jest/go-test output was observed (PASS=all green, FAIL=any failures, N/A=no test framework invoked).`,
    { label: `verify:${c.id}`, phase: 'Verify', schema: ACCEPTANCE_SCHEMA }
  ),
  (verdict, c) => parallel(
    Array.from({ length: params.refuteVoters }, (_, i) => () =>
      agent(
        `Try to REFUTE this acceptance verdict for criterion ${c.id}: "${c.description}". Read ${args.specPath} and ${args.planPath}; inspect the changed implementation in ${args.repositoryRoot} from ${args.baseSha}..HEAD. Verdict: ${JSON.stringify(verdict)}. Derive the required observables from SPEC independently, then check whether the executed assertions and observations prove them. Presence-only evidence cannot prove required structure or behavior; a claimed PASS is not evidence. Inspect the evidence; if you find ANY hole, set real=false. Default to refuted (real=false) when uncertain. Voter ${i + 1}/${params.refuteVoters}.`,
