...
// Design gate (canonical: skills/shared/implementer-contract.md). Named in every
// implementer prompt because a SessionStart hook does not reach a Workflow-dispatched
// agent. MUST stay outside the @inject blocks above.
const fourQuestions = (libDir) =>
  'FOUR QUESTIONS (design gate — on by default). Before implementing and again before ' +
  'DONE, ask of the change: can I make it more modular? can I make it more extensible? ' +
  'is this the least amount of code that makes it happen? does this hold at production ' +
  'scale (memory and work bounded against deployment-sized input, not the fixture)? ' +
  'Full contract: ' +
  (libDir ? libDir + '/../skills/shared/implementer-contract.md' : 'skills/shared/implementer-contract.md') +
  '.'

// Ponytail laziness-ladder directive (canonical: skills/shared/laziness-ladder.md).
// Named in every implementer prompt for the same reason.
...
const EXECUTION_DISCIPLINE =
  ...
  'criterion you cannot meet is a loud failure with evidence, never a note. Keep extras ' +
  'out: leave a pre-existing bug, performance concern, or unrelated behavior unchanged ' +
  'unless the requested behavior cannot work without it. Keep permanent tests to requested ' +
  'behavior or the repository\'s established convention, and surgically edit needed lines ' +
  'instead of rewriting a whole file when the result is unchanged.'

const writingGoodTests = (libDir) =>
  'WRITING GOOD TESTS. Read ' +
  (libDir ? libDir + '/../skills/shared/writing-good-tests.md' : 'skills/shared/writing-good-tests.md') +
  ' before adding or changing a test — do not paste it. Name the break; no string-presence ' +
  'traps; no change detectors. TDD: code-producing tasks write the failing test FIRST, ' +
...
// Named in every implementer prompt for the same reason.
const engineeringDirectives = (libDir) =>
  'ENGINEERING DIRECTIVES (on by default). Read ' +
  (libDir ? libDir + '/../skills/shared/engineering-directives.md' : 'skills/shared/engineering-directives.md') +
  ' — do not paste it. Simple over clever: the construct the next reader decodes without ' +
  'a comment. Idiomatic for the version the repo pins. Versions come from a tool ' +
  '(manifest, package manager, registry, advisory check), never from recall; report ' +
  'version: <name>@<v> source: <command> or unverified. Name the scaling input before ' +
...
${fourQuestions(skillDir ? `${skillDir}/../../lib` : '')}
...
${writingGoodTests(skillDir ? `${skillDir}/../../lib` : '')}

${engineeringDirectives(skillDir ? `${skillDir}/../../lib` : '')}
