...
const CODE_FOR_HUMANS =
  ...
  'to deviate. Where the convention is unclear, measure it: run `bash lib/house-style.sh ' +
  'probe <files>` for comment density, doc-comment usage, indentation, and naming case ' +
  'from the actual neighbors. Comments carry WHY, never what: a constraint not visible ' +
...
After every passed task in this wave has been published and only if no integration
failure occurred, run this repository-wide candidate gate ONCE against the integrated
feature branch:

  bash ${shellQuote(`${skillDir}/../../lib/feature-validation.sh`)} compare ${shellQuote(waveCandidateDir)}

If it exits nonzero, set integrationFailure to { taskId: the first merged task id,
reason: "candidate-validation-failed", detail: "feature-validation compare failed" }.
Do not claim the wave is complete. Each task's focused verify command above still ran
after any rebase; this one command validates the exact integrated wave candidate.
