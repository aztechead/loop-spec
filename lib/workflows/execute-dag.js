...
// Workflow-dispatched agent. MUST stay outside the @inject blocks above. Contract paths
// come from the injected skillDir and fall back to repo-relative names; probe paths need it.
const engineeringContract = (libDir) =>
  'ENGINEERING CONTRACT (on by default; every directive binds). The index is ' +
  '' + (libDir ? libDir + '/../skills/shared/engineering-directives.md' : 'skills/shared/engineering-directives.md') + '. Read these before writing code, never paste them: ' +
  '' + (libDir ? libDir + '/../skills/shared/implementer-contract.md' : 'skills/shared/implementer-contract.md') + ' (FOUR QUESTIONS (design gate): can I make it more modular? ' +
  ...
  'input, not the fixture?); ' + (libDir ? libDir + '/../skills/shared/laziness-ladder.md' : 'skills/shared/laziness-ladder.md') + ' (ponytail laziness ladder: YAGNI, then DRY, reuse ' +
  'what is already here); ' + (libDir ? libDir + '/../skills/shared/design-for-change.md' : 'skills/shared/design-for-change.md') + ' (seams, not speculation); ' +
  '' + (libDir ? libDir + '/../skills/shared/human-code.md' : 'skills/shared/human-code.md') + ' (house style over habit: read the neighbors, comments carry WHY, ' +
  'density matches the file, never cut simplicity: markers; CODE A HUMAN CAN OPERATE: fail ' +
  'loudly, or say why not); ' + (libDir ? libDir + '/../skills/shared/human-docs.md' : 'skills/shared/human-docs.md') + ' (DOCS FOR HUMANS: one job per document, ' +
  ...
  'headings, EVID lines, or licenses); ' + (libDir ? libDir + '/../skills/shared/writing-good-tests.md' : 'skills/shared/writing-good-tests.md') + ' (WRITING GOOD TESTS: ' +
  ...
  '\n' +
  'Before DONE, on <files you touched>: bash ' + libDir + '/indirection-scan.sh scan (one-caller ' +
  'helpers to inline); bash ' + libDir + '/duplication-scan.sh scan (duplicate= same lines, ' +
  'similar= names changed; both count); bash ' + libDir + '/house-style.sh compare; ' +
  'bash ' + libDir + '/comment-tells.sh scan; bash ' + libDir + '/failure-tells.sh scan; ' +
  'bash ' + libDir + '/doc-tells.sh scan <markdown you touched>.\n' +
  '\n' +
  ...
  '\n' +
  'EXECUTION DISCIPLINE (evidence over recall). Read ' + (libDir ? libDir + '/../skills/shared/execution-discipline.md' : 'skills/shared/execution-discipline.md') + ', do not ' +
...
const NO_PREJUDGE =
  'Read skills/shared/review-prompts/no-prejudge.md (do not paste). Never tell yourself ' +
  'what a finding is worth before you read the diff. A plan-mandated finding is reported, ' +
  'not skipped.'
...
// Workflow callers must pass the effective cap from resource-bounds.sh. Keep a
// serial fallback for older callers, but reject malformed values instead of
// letting Array.slice treat them as an accidental unbounded wave.
const effectiveMaxParallelImplementers = maxParallelImplementers ?? 1
if (!Number.isInteger(effectiveMaxParallelImplementers) || effectiveMaxParallelImplementers < 1) {
  throw new Error('maxParallelImplementers must be a positive integer')
}
...
while (true) {
  ...
  const wave = ready.slice(0, effectiveMaxParallelImplementers)
  ...
  const waveResults = await parallel(wave.map((taskId, waveIdx) => async () => {
    // Integration stops at the first failure. Other approved candidates still
    // await publication; do not spend their implementation budget again.
    if (approvedCandidates[taskId]) return approvedCandidates[taskId]
    ...
    let priorFindings = integrationFindings[taskId] || []

    for (let attempt = attemptsUsed[taskId] || 0; attempt < maxRetriesPerTask; attempt++) {
      attemptsUsed[taskId] = attempt + 1
...
${engineeringContract(skillDir ? `${skillDir}/../../lib` : '')}
...
If its assertion contradicts the acceptance criteria, report the exact contradiction
and output. Do not change correct product behavior to satisfy a faulty generated check.
...
Over-engineering pass (ponytail): scan the diff for complexity it does not need and flag each as a rework finding -- delete (dead/speculative code), stdlib (hand-rolled thing the standard library ships), yagni (abstraction with one implementation / config nobody sets), shrink (same logic, fewer lines). The ponytail minimum (a single smoke/assert check or an accepted simplicity:-marked shortcut) is the floor of this pass, not a finding.

A requirement that lives in unchanged code or spans tasks is not a fail: put it in unverified[] with why the diff cannot show it. Non-empty unverified on pass is treated as rework.
...
Return JSON: { verdict: "pass"|"rework"|"block", findings: ["<finding 1>", ...], unverified: [{"requirement":"...","why":"..."}] }`,
      ...
      if (reviewVerdict.verdict === 'pass') {
        const unverified = Array.isArray(reviewVerdict.unverified) ? reviewVerdict.unverified : []
        if (unverified.length > 0) {
          // Downgrade before retrying: an unverified item must not evaporate into a
          // merge when this was the last attempt.
          reviewVerdict = { ...reviewVerdict, verdict: 'rework' }
          priorFindings = (reviewVerdict.findings || []).concat(
            unverified.map((u) => `unverified: ${u.requirement} (${u.why})`)
          )
          continue
        }
        break
    ...
    if (finalVerdict === 'pass' && committed) approvedCandidates[taskId] = result
    return result
  }))
...
Return JSON:
{
  ...
  if (mergeResult) {
    ...
    if (mergeResult.integrationFailure) {
      ...
      if (failure.reason === 'verify-failed') {
        const task = byId[failure.taskId]
        const diagnosis = await agent(
          `Diagnose integration verification for task ${failure.taskId}. Do not edit any files.
Read the task worktree at ${worktreeBase}/task-${failure.taskId} and the specification
at ${task.specPath || `${featureWorktreeRoot}/.loop-spec/features/${slug}/SPEC.md`}.
Task: ${JSON.stringify(task)}
Recorded command: ${task.verifyCommand}
Integration failure: ${failure.detail}
Compare the exact command and its failure with the unchanged acceptance criteria.
Return kind=product only when concrete evidence demonstrates a product defect under
those criteria. Return kind=check when the generated check contradicts the criteria,
and kind=uncertain when the evidence is insufficient. Do not change correct product
behavior to satisfy a faulty check. Include the evidence in detail.`,
          { label: 'integration-diagnosis', phase: 'Implement', schema: {
            type: 'object', required: ['kind', 'detail'], properties: {
              kind: { enum: ['product', 'check', 'uncertain'] }, detail: { type: 'string' },
            },
          } }
        )
        if (diagnosis?.kind !== 'product' || !diagnosis.detail?.trim()) {
          // Keep candidates/worktrees intact for the lead's one-shot repair-verify
          // path. A repaired command must receive a new package and review.
          escalation = {
            taskId: failure.taskId,
            reason: 'verify-command-repair-required',
            detail: `${failure.detail}. ${diagnosis?.detail || 'Diagnosis unavailable.'} Stop for repair-verify; preserve criteria and attempt counts, then package and review again before integration.`,
          }
          break
        }
        delete approvedCandidates[failure.taskId]
        if (attemptsUsed[failure.taskId] >= maxRetriesPerTask) {
          blocked.push({ taskId: failure.taskId, reason: 'retry-exhausted' })
        } else {
          integrationFindings[failure.taskId] = [
            `Integration verification failed. Run the recorded command exactly: ${byId[failure.taskId].verifyCommand}. ${failure.detail}. ${diagnosis.detail}`,
          ]
        }
      } else {
        escalation = {
