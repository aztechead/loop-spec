...
Step 1.5 - Prepare declared dev/test dependencies in the task worktree:
  bash ${shellQuote(`${skillDir}/../../lib/prepare-environment.sh`)} run --root ${shellQuote(taskWorktreePath)} --command ${shellQuote(commands.prepare || '')}
...
Return JSON: { verdict: "pass"|"rework"|"block", findings: ["<finding 1>", ...] }`,
  ...
  const mergeSteps = passed.map((p, i) => {
    const candidateFeatureDir = `${p.worktreePath}/.loop-spec/features/${slug}`
    const candidateVerify = `${byId[p.taskId].verifyCommand || 'true'} && bash ${shellQuote(`${skillDir}/../../lib/feature-validation.sh`)} compare ${shellQuote(candidateFeatureDir)}`
