...
const codeForHumans = (libDir) =>
  ...
  'When this code breaks at 03:00 the person on call has only what it said: never swallow ' +
  'an error (a handler that catches and does nothing erases the one record of what ' +
  'happened -- log it, re-raise it, or state why the failure is uninteresting), never exit ' +
  'non-zero in silence, and make each error message name what broke and the next move ' +
  'where you know it' +
  (libDir
    ? '. Before DONE run `bash ' + libDir + '/failure-tells.sh scan <files you touched>`, ' +
      'which flags those three shapes and stays quiet on the deliberate ones.'
    : '.')
...
const docsForHumans = (libDir) =>
  'DOCS FOR HUMANS (the markdown is a deliverable too — on by default). A person maintains ' +
  'and operates every document you write, long after this run ends. Name its reader in the ' +
  ...
  'says why. A procedure states its prerequisites, then the exact copy-pasteable command, ' +
  'then what success looks like, then what to do when the step fails. Cite, never copy: ' +
  'point at file:line instead of restating what the code says — stale prose is worse than ' +
  'none, because it is wrong with authority. If your change makes a document false (README, ' +
  'help text, runbook, config table), fix it IN THIS DIFF; a follow-up documentation task is ' +
  'deferred scope. Never invent a documentation convention this repository does not already ' +
  'have. ' +
  (libDir
    ? 'Before DONE run `bash ' + libDir + '/doc-tells.sh scan <the markdown you touched>`: ' +
      'it flags a relative link with no target, an inline-code path the tree no longer ' +
      'holds, and a command holding a placeholder your prose never explains. '
