...
if (args && args.dryRun) {
  return {
    domains: (args.staleDomains || ['arch']).map(d => ({
      name: d, mdPath: `docs/super-spec/codebase/${d}.md`, coverage: 0.9, weakSpots: []
    })),
    dryRun: true,
  }
}
...
const mapped = await parallel(stale.map(d => () =>
  agent(
    `Refresh the ${d} domain map. Read docs/super-spec/codebase/${d}.md, refresh from current repo state, write the updated markdown back to that file, then return the structured summary.`,
...
if (params.completenessCritic) {
  ...
  if (critic && critic.remap && critic.remap.length) {
    const remapped = await parallel(critic.remap.map(d => () =>
      agent(
        `Re-map the ${d} domain with extra attention to weak coverage areas. Update docs/super-spec/codebase/${d}.md and return the updated summary.`,
...
return { domains, tier: args.tier || 'balanced', staleDomainsRequested: stale }
