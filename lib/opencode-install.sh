...
install_cmd() {
  ...
  #     namespaced, so the skill tool path is untouched.
  mkdir -p "$TARGET/commands/loop-spec"
  local wrappers_out
  wrappers_out="$(python3 - "$REPO_ROOT/skills" "$TARGET/commands/loop-spec" <<'PYEOF'
...
for name in sorted(os.listdir(src_root)):
    ...
    body = [
        ...
        "",
        f"Load the loop-spec skill \"{skill}\" with the native skill tool — call "
        f"skill({{ name: \"{skill}\" }}) — then follow the loaded skill "
        "instructions.",
        "",
    ]
    if hint:
...
print("\n".join(written))
PYEOF
)" || _die2 "skill command wrapper generation failed"
  while IFS= read -r p; do
    [[ -n "$p" ]] && CREATED+=("$p")
  done <<< "$wrappers_out"
