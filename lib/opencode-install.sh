...
#                agents inherit the session model unless `install --model`
#                pins a role to a provider/model id.)
#
...
#       [--model <role|adversarial>=<provider/model>]...
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --model)
      ...
      route="${2:-}"
      [[ -n "$route" && "$route" == *=* ]] || _die2 "--model needs role=provider/model"
      role="${route%%=*}"
      model="${route#*=}"
      [[ "$role" =~ ^[a-z0-9-]+$ ]] || _die2 "invalid model role '$role'"
      [[ "$model" =~ ^[^/[:space:]]+/[^[:space:]]+$ ]] \
        || _die2 "model route for '$role' must be provider/model (got '$model')"
...
for fname in sorted(os.listdir(src_dir)):
    ...
    routed_model = routes.get(role)
    if routed_model is None and role in adversarial_roles:
        routed_model = routes.get("adversarial")
    if routed_model:
        lines.append(f"model: {routed_model}")
    ...
    model_note = (f"routed to `{routed_model}`" if routed_model
                  else "inherits the session model")
    header = (
        "<!-- GENERATED by loop-spec's lib/opencode-install.sh from "
        f"agents/{fname} — edit the source, not this file. Claude Code "
        f"model selector was `{data.get('model', '')}`; this agent {model_note}. -->"
    )
...
with open(readonly, "w", encoding="utf-8") as f:
    readonly_model = routes.get("readonly")
    readonly_model_line = f"model: {readonly_model}\n" if readonly_model else ""
    f.write("""---
description: Read-only loop-spec compiler and judge for headless OpenCode runs.
mode: primary
hidden: true
""" + readonly_model_line + """permission:
  "*": deny
  bash: deny
  edit: deny
  task: deny
...
import hashlib, json, os, sys
version, output, routes_json, *paths = sys.argv[1:]
routes = json.loads(routes_json)
artifacts = []
for path in paths:
