...
canonical_models() {
  ...
  local v_iterateJudge v_codeReviewer v_implementer v_verifier
  if [[ -n "$phase" ]]; then
...
validate_routing() {
  ...
  for role in SPEC_WRITER PLANNER ADVOCATE CHALLENGER ROUTE_JUDGE SPEC_COMPLIANCE_REVIEWER \
              ITERATE_JUDGE CODE_REVIEWER IMPLEMENTER VERIFIER; do
...
common_skeleton() {
  ...
  jq -n \
    ...
    '{
      ...
      artifacts: {
        specInterview: null,
        spec: null, plan: null, execution: null, verification: null,
        iteration: null
      },
