...
#
# Why: the bundled surface is over a hundred scripts under `lib/` and `hooks/`, plus every
# shared contract and role charter. A phase agent that needs one of them has no cheaper move than
...
usage() {
  echo "usage: surface.sh find <term> [term ...]   entries matching EVERY term" >&2
  echo "       surface.sh show <path-or-name>      one entry's header block" >&2
  echo "       surface.sh covers <path> [path ...] registered suites naming each path" >&2
  echo "       surface.sh list [lib|shared|agent|all]" >&2
  exit 2
}
