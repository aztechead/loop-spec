...
A repeat call on the same HEAD and commands returns the stored result (`.verify.cached`) without re-running the suite.
...
to SPEC. EXECUTE publishes the complete intake before acknowledging it; malformed
