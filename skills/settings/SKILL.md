...
|---|---|---|---|---|---|
| `grill` | ON | `grill.conf` | `hooks/team/grill-inject.sh` | `LOOP_SPEC_GRILL=0` | 2-4 sharp disambiguation questions right after the opening request. Inside the cycle, SPEC's interview continues the grill and its own design-lock step still runs the design-shape clarifying loop unless the run is autonomous (`execStyle: auto` is not autonomous mode). |
