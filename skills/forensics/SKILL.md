...
Diagnose the run without changing source code or feature state.
Write only `.loop-spec/forensics/report-{ISO-8601}.md` and terminal-result telemetry.
...
When a file is missing or a command fails, record the evidence gap and continue.
Do not treat unavailable evidence as proof that the run is healthy.
...
1. {Specific action, such as "Run /loop-spec:cycle to resume the feature" or
   "Inspect the phase's FLAG lines before resuming"}
2. {Additional step if applicable}

---

*Report generated by /loop-spec:forensics. Report path: .loop-spec/forensics/report-{timestamp}.md*
```
...
{if count > 0: "Run /loop-spec:rollback or /loop-spec:cycle to act on findings."}
```
...
The `forensics/report-` prefix identifies this skill's report. Terminal-result telemetry is the only other output.
