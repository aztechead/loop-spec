...
pending, which is every autonomous and non-interactive run and every re-invocation
that continues a feature. Print each line of `.notices[]` and `.warnings[]`, nothing
else. Exit 3 is an abort whose message is already on stderr: relay it and stop. Read
`.action`:
...
- `NEXT phase=<p> label="..." effort=<system1|system2>` — print it, treat every
  ...
  invoke `Skill(loop-spec:<p>)`. `effort=system1` means keep the phase direct;
  `system2` means state assumptions and check their evidence first. When the phase
  skill returns, close it:
  ...
  and act on that answer with the same list. `REDO` is the only answer that keeps
  this invocation going: the same phase, until its exit is clean or the driver
  escalates.
...
- `HANDOFF next=<p> model=<m>` or `REWIND next=<p>` — the phase is closed and the next
  ...
  (`hooks/team/nested-session-guard.sh` denies the launch). For a Claude worktree feature,
...
carries only task and artifact commits), the paused `phase-handoff` result, and the
autonomous checkpoint PR; phase skills never write `currentPhase`.
...
product) is declined BEFORE the tree changes with the `write-terminal` snippet in
...
One invocation is one phase, so a headless caller loops: re-invoke while
`.loop-spec/last-result.json` says `status=paused` with `reason=phase-handoff`, and
stop on any other status. `docs/loop-spec/cloud-run-autonomous.md` is the bounded
controller for Claude Code; the eval driver (`evals/eval_run.py`) is the same loop.
