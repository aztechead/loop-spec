...
5. **Implement** the task in the worktree at `{worktreeBase}/task-<id>/`. (Create the worktree on first claim; the worktree persists across rework rounds for the same task.)
   ...
   - **Code for humans (house style over habit)** (`skills/shared/human-code.md`): the diff must read like the code around it. Read the neighbors of every path in `metadata.files` first and match them — naming, error idiom, test structure, layout, import order; the house convention outranks your defaults, and disagreeing with it is a finding, never a licence to deviate. Measure rather than guess with `bash lib/house-style.sh probe <files>`. Comments carry WHY, never what; comment density matches the file, not an absolute; a good name deletes a comment. Never narrate the code, announce the edit, or narrate history — `bash lib/comment-tells.sh scan <files>` catches those three. Never cut `simplicity:` markers, file-header purpose blocks the codebase uses, TODO/FIXME/NOTE/HACK/SAFETY markers, or any comment encoding a non-obvious why.
