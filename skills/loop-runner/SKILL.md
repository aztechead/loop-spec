...
resume; raise `--max-iterations` to extend a halted run; `--reset` to start clean.
...
```json
{
  ...
  "halt_reason": "complete | max_iterations | timeout | no_progress |
                  verifier_thrash | verifier_integrity | agent_error",
  "iterations": 7, "total_turns": 41,
...
model paper over it. The supervisor requires a clean tree to start
...
Single well-defined edits — just do them. Loops earn their keep when a task needs
...
`bash tests/run_tests.sh` — offline regression suite (43 checks) using
