...
# The registry of default phase adapters: phase -> (kind, target). "lead": the lead
# runs role `target` itself (defaults.run_lead_phase). "stepped": module `target`
# exposes step()/on_submit() and, optionally, on_step_refused(). "run": module
# `target` exposes run(), one pass with no worker step (deliver). The only list of
# phases the controller drives; modules are imported by name, lazily, because every
# adapter imports this module.
...
def default_adapter(phase: str):
    """The module behind `phase`'s default implementation, or None for a lead phase."""
    kind, target = DEFAULT_IMPLEMENTATIONS[phase]
    return None if kind == "lead" else importlib.import_module(f"loop_spec.{target}")
...
def _run_default_stepped(module, store, paths, attempt_dir: Path, product_path: Path) -> tuple[int, str | None]:
    ...
    # docstrings say wiring them in is not their job). Every adapter returns the step
    # contract's types from steps.py (imported here: steps.py imports this module).
    ...
    if isinstance(outcome, steps_module.Product):
        atomic_write_json(product_path, outcome.product)
        return 0, "product"
    if isinstance(outcome, steps_module.IssueSteps):
        atomic_write_json(attempt_dir / "steps.json", outcome.requests)
        return 2, "steps"
    if isinstance(outcome, steps_module.IssueStep):
        atomic_write_json(attempt_dir / "step.json", outcome.request)
        return 2, "step"
    if isinstance(outcome, steps_module.Wait):
...
def _run_default_deliver(store, paths, attempt_dir: Path, product_path: Path) -> int:
    ...
    from loop_spec import steps as steps_module
    ctx = read_json(attempt_dir / "context.json")
    outcome = default_adapter("deliver").run(store, paths, ctx)
    if isinstance(outcome, steps_module.Product):
        atomic_write_json(product_path, outcome.product)
        return 0
    atomic_write_json(attempt_dir / "question.json", outcome.question_request)
...
def invoke(paths, *, phase: str, attempt_id: str, implementation: str, program_launcher: Path, store=None) -> PhaseOutcome:
    ...
    if implementation == "default":
        ...
        if kind == "stepped":
            code, stepped_kind = _run_default_stepped(default_adapter(phase), store, paths, attempt_dir, product_path)
        elif kind == "run":
            ...
            code = defaults.run_lead_phase(phase, target, attempt_dir / "context.json", product_path)
