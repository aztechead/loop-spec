...
**Why this analog (one line):** Deleting `loop-spec-mapper-arch.md` and `loop-spec-mapper-tech.md` drops the agent count from 14 to 12; the validator's hard-coded constant must be updated in the same commit.
...
- The `restrict-agent-paths.sh` hook uses `loop-spec-mapper-*` glob pattern (line 93), which does NOT need updating when specific mapper files are deleted - the glob still covers the remaining three mappers.
...
**Closest analog:** `agents/loop-spec-planner.md:55-63` (the "Role boundary" section with read instructions)
...
```markdown
<!-- agents/loop-spec-planner.md:55-63 (existing role boundary - add graphify preference above) -->
## Role boundary
...
<!-- grep -n 'graphify.*query\|graphify.*path\|graphify.*explain\|graphify-out/wiki' agents/loop-spec-planner.md -->
<!-- grep -n 'graphify.*query\|graphify.*path\|graphify.*explain\|graphify-out/wiki' agents/loop-spec-pattern-mapper.md -->
```
...
grep -n 'graphify' agents/loop-spec-planner.md
grep -n 'graphify' agents/loop-spec-pattern-mapper.md
```
...
- The new block should live inside `.loop-spec/codebase/index.json` schema documentation (which is already documented in `feature-state-schema.md`), not inside `feature.json` schema.
