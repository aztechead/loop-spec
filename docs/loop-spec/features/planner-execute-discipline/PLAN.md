...
**Spec:** `docs/loop-spec/features/planner-execute-discipline/SPEC.md`
**Created:** 2026-05-28
...
- `hooks/team/strategy-rotation.sh` reads `tool_name` and failure status from hook stdin (PostToolUse payload). It uses `CLAUDE_CODE_SESSION_ID` then `CLAUDE_SESSION_ID` then `$$` as the session key. State file path: `/tmp/loop-spec-failures-${SESSION}.json`.
- `hooks/team/budget-gate.sh` reads cost from `metrics-session.json` (field `.totals.estimated_cost_usd`) if the file exists at the current working directory, then falls back to `LOOP_SPEC_CURRENT_COST_USD`. If neither is present, it exits 0 unconditionally.
...
- Create: `docs/loop-spec/planner-antipatterns.md` - do/don't reference for planner acceptance criteria
- Modify: `agents/loop-spec-planner.md` - add BANNED PHRASES section + read_first[] requirement
- Modify: `agents/loop-spec-spec-writer.md` - require `<decisions>` block in SPEC.md output
...
|----|---------|-----------|-------|-----------|
| task-001 | anti-shallow planner rules and template update | - | agents/loop-spec-planner.md, skills/shared/artifact-templates/PLAN.md.template, docs/loop-spec/planner-antipatterns.md | medium |
| task-002 | spec-writer decisions-block requirement | - | agents/loop-spec-spec-writer.md | small |
...
- `docs/loop-spec/planner-antipatterns.md`

**read_first:**
- `agents/loop-spec-planner.md`
- `skills/shared/artifact-templates/PLAN.md.template`
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (section "User-facing behavior", first paragraph)

**Verify:** `grep -c "read_first" agents/loop-spec-planner.md` returns 1 or more; `grep -A 10 "BANNED\|MUST NOT" agents/loop-spec-planner.md | grep -c "looks correct\|properly configured"` returns 1 or more; `grep -c "read_first:\|- \[ \]" skills/shared/artifact-templates/PLAN.md.template` returns 2 or more; `grep -c "DO NOT\|BAD\|GOOD\|Instead\|instead" docs/loop-spec/planner-antipatterns.md` returns 1 or more; `grep -rP "\xe2\x80\x94" agents/loop-spec-planner.md docs/loop-spec/planner-antipatterns.md skills/shared/artifact-templates/PLAN.md.template` exits 1 (no matches).

**Acceptance criteria:**
- [ ] `agents/loop-spec-planner.md` gains a section titled "BANNED PHRASES" or "MUST NOT appear in acceptance criteria" that explicitly lists "looks correct", "properly configured", "consistent with", "align X with Y", "matches Y", and "well-formed" (without a schema reference).
- [ ] `agents/loop-spec-planner.md` gains a section titled "REQUIRED CONCRETE FORM" or equivalent that states every acceptance criterion must contain at least one of: an exact value, a regex pattern, an exit code, a file path, a grep command, or a JSON path expression.
- [ ] `agents/loop-spec-planner.md` states that every task must include a `read_first:` field (empty list is allowed) containing concrete file identifiers.
...
- [ ] `docs/loop-spec/planner-antipatterns.md` exists and contains at least 3 do/don't pairs illustrating banned vs. required forms (e.g., "looks correct" vs. "`grep -c 'pattern' file` returns 1").
...
- [ ] Step 1: Read `agents/loop-spec-planner.md` fully. Identify the "Role boundary" and "What NOT to do" sections as insertion points.
- [ ] Step 2: Edit `agents/loop-spec-planner.md` to add the BANNED PHRASES section (after "Role boundary") and the REQUIRED CONCRETE FORM rule (after BANNED PHRASES). Add the `read_first:` field requirement to the per-task field list in "Role boundary".
...
- [ ] Step 5: Write `docs/loop-spec/planner-antipatterns.md` with at least 6 do/don't pairs (one per banned phrase). Structure as a flat markdown doc with `## DO NOT` and `## DO` subsections or a paired table.
...
**Goal:** Update `agents/loop-spec-spec-writer.md` to require that SPEC.md output includes a `<decisions>` XML block.
...
**read_first:**
- `agents/loop-spec-spec-writer.md`
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (the `<decisions>` block in that file itself serves as the canonical example)

**Verify:** `grep -c "<decisions>" agents/loop-spec-spec-writer.md` returns 1 or more; `grep -rP "\xe2\x80\x94" agents/loop-spec-spec-writer.md` exits 1 (no matches).

**Acceptance criteria:**
- [ ] `agents/loop-spec-spec-writer.md` contains the string `<decisions>` in at least one location that binds it as a required output section (not just a passing reference).
...
- [ ] Step 1: Read `agents/loop-spec-spec-writer.md`.
...
- [ ] Step 4: Run `grep -c "<decisions>" agents/loop-spec-spec-writer.md` and confirm result >= 1.
- [ ] Step 5: Commit.
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (Success criteria lines 94-101 for exact verify invocations)
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (Success criteria line 106: exact verify invocation using `/dev/stdin` heredoc)
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (User-facing behavior, "Post-merge build/test gate" paragraph: exact probe list -- Makefile, package.json, Cargo.toml, pyproject.toml, setup.py, go.mod)
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (Decision 2: advisory on quick, blocking on quality/balanced)
...
- [ ] Step 2: Edit Step 5 to insert the gate prose block before the `git add` line. The inserted block: invokes `bash "${CLAUDE_PLUGIN_ROOT}/lib/decision-coverage.sh" "{spec_path}" "docs/loop-spec/features/{slug}/PLAN.md"`, captures exit code, and describes the tier-conditional branching.
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (Decision 3: plan-adherence; Decision 6: post-merge test gate skipped on quick, runs on quality/balanced)
...
**Goal:** Create `hooks/team/strategy-rotation.sh` (PostToolUse:Bash|Edit|Write) and its test suite. The hook tracks consecutive tool failures per tool per session, emits an `additionalContext` rotation prompt at threshold, resets on success, and honors `LOOP_SPEC_STRATEGY_ROTATION=0` kill-switch.
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (Success criteria lines 117-127 for exact test case names)
...
- [ ] State file is `/tmp/loop-spec-failures-${SESSION}.json` where SESSION is resolved from `CLAUDE_CODE_SESSION_ID`, then `CLAUDE_SESSION_ID`, then `$$`.
- [ ] Consecutive-failure threshold defaults to 2, overridden by `LOOP_SPEC_STRATEGY_ROTATION_THRESHOLD`.
...
- [ ] `LOOP_SPEC_STRATEGY_ROTATION=0` causes the hook to exit 0 without reading stdin or state.
...
**Goal:** Create `hooks/team/budget-gate.sh` (PreToolUse:Agent) and its test suite. The hook reads session cost, emits an `additionalContext` warning at 80-99% of ceiling, exits 2 (block) at 100%+, exits 0 unconditionally when `LOOP_SPEC_MAX_COST_USD` is unset or `LOOP_SPEC_BUDGET_GUARD=0`.
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (Decision 5: cost source hierarchy; Success criteria lines 129-142)
...
- [ ] When `LOOP_SPEC_MAX_COST_USD` is unset, the hook exits 0 unconditionally (no stdin drain required, but must not error).
- [ ] When `LOOP_SPEC_BUDGET_GUARD=0`, the hook exits 0 unconditionally.
- [ ] Cost is read from `metrics-session.json` field `.totals.estimated_cost_usd` if the file exists in CWD; otherwise from `LOOP_SPEC_CURRENT_COST_USD` env var; otherwise treated as 0.
- [ ] At 80-99% of `LOOP_SPEC_MAX_COST_USD`, exits 0 and emits `{"additionalContext":"WARNING: session cost is at ...% of ceiling ..."}` to stdout.
- [ ] At 100%+ of `LOOP_SPEC_MAX_COST_USD`, exits 2 with a block message to stderr naming the current cost and ceiling.
...
- [ ] Step 1: Write `hooks/team/budget-gate.test.sh` with at least 5 cases. Use temp dirs and `LOOP_SPEC_CURRENT_COST_USD` env var (simpler than writing a metrics-session.json in each test). Cases: warn-at-80 (cost=8, max=10 -> stdout contains additionalContext, exit 0), block-at-100 (cost=10, max=10 -> exit 2), no-ceiling (MAX unset -> exit 0), kill-switch (GUARD=0 -> exit 0), fail-open (metrics-session.json contains invalid JSON -> exit 0). Script does not exist -- all FAIL.
- [ ] Step 2: Run tests. Confirm FAIL.
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (Success criteria line 144-145: exact jq verify commands)
...
- `docs/loop-spec/features/planner-execute-discipline/SPEC.md` (Success criteria lines 147-157 for exact grep patterns)
