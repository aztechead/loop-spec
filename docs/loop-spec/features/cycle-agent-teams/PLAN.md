...
> Produced by `loop-spec-planner`. Read by EXECUTE phase implementers.
...
- **SPEC**: `docs/loop-spec/features/cycle-agent-teams/SPEC.md`
- **PATTERNS**: `docs/loop-spec/features/cycle-agent-teams/PATTERNS.md`
- **Codebase mapping**: `docs/loop-spec/codebase/{TECH,ARCH,QUALITY,CONCERNS,DOMAIN}.md`
...
1. **`.gitignore` resolution.** PATTERNS open question on `.loop-spec/codebase/index.json` shadowing: we resolve by adding two specific paths to `.gitignore` instead of a blanket `.loop-spec/`: `/.loop-spec/features/` and `/.loop-spec/worktrees/`. This preserves `.loop-spec/codebase/index.json` (currently tracked, per `skills/map-codebase/SKILL.md:84`) without needing a negation rule. The SPEC's success-criterion `git status --ignored` check is satisfied because `.loop-spec/features/{slug}/feature.json` is under one of the ignored prefixes.
...
7. **`agents/loop-spec-*.md` count.** `validate-agents.sh` currently hard-codes `EXPECTED=14`. The agent set is reused as-is (no new agent files), so `EXPECTED` is unchanged. The frontmatter rule is purely additive.
...
### task-001: gitignore + .loop-spec scaffolding
- **Files**: `.gitignore`
- **Verify**: `bash -c 'grep -q "^/.loop-spec/features/$" .gitignore && grep -q "^/.loop-spec/worktrees/$" .gitignore'`
- **Acceptance**: `.gitignore` contains `/.loop-spec/features/` and `/.loop-spec/worktrees/` lines; `git check-ignore .loop-spec/features/x/feature.json` reports the path is ignored.
- **BlockedBy**: none
- **Wave**: 1
- **Steps**:
  1. Read existing `.gitignore`.
  2. Append `/.loop-spec/features/` and `/.loop-spec/worktrees/` (per Assumption 1).
  3. Run `mkdir -p .loop-spec/features/dummy && git check-ignore .loop-spec/features/dummy/feature.json && rmdir .loop-spec/features/dummy`.
...
- **Files**: `docs/loop-spec/PREREQUISITES.md` (new)
- **Verify**: `grep -q CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS docs/loop-spec/PREREQUISITES.md`
...
- **Wave**: 2
- **Steps**:
  1. **Write failing test first.** `tests/lib/team-ops.test.sh` asserts: `team_name_for_phase discuss foo` -> `loop-spec-discuss-foo`; `assert_team_env` exits 0 when `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`, exits 2 with the documented message otherwise; `feature_json_path foo` -> `.loop-spec/features/foo/feature.json`.
...
- **Steps**:
  ...
  4. Step 2 (continued): add the harness-capability probe described in SPEC `## User-facing behavior` item 1: create `loop-spec-probe-{pid}`, exercise `TaskUpdate`+`metadata`+`SendMessage`+concurrent self-claim+`TeamDelete`. On any failure, emit the documented PROBE FAILED block and abort.
...
- **Steps**:
  ...
  3. Step 6 routing: add `TeamCreate({name: loop-spec-{phase}-{slug}, teammates: [...]})` before each phase Skill call. Mirror `currentTeamName` write pattern. After phase completes, `TeamDelete` and clear `currentTeamName`.
...
- **Steps**:
  ...
  3. Step 1: dispatch via `TeamCreate({name: loop-spec-discuss-{slug}, teammates: [{name: spec-writer-1, agent: loop-spec-spec-writer, model, prompt}]})` per PATTERNS Concept 3 gotcha mapping.
  ...
  5. Round handling: append each round-end message to `.loop-spec/features/{slug}/gate-logs/discuss-round-{N}.md`. Persist round counter via `lib/feature-write.sh` to `feature.json.currentGate`.
...
- **Steps**:
  ...
  3. Step 0 (entry): per-entry pre-task file-conflict detection per SPEC `#### EXECUTE team` "Pre-task file-conflict detection". Read exclusion list from `feature.json.fileConflictExcludeGlobs[]` and `.loop-spec/file-conflict-exclude.txt` (union). Default empty.
...
- **Steps**:
  ...
  3. Step 1: `TeamCreate({name: loop-spec-verify-{slug}, teammates: [{name: verifier-1, ...}, {name: code-reviewer-1, ...}, ...mappers if stale]})`.
...
- **Steps**:
  ...
  2. Frontmatter and Inputs migration.
  3. `TeamCreate({name: loop-spec-map-codebase-{project-id}, teammates: [{mapper-tech, mapper-arch, mapper-quality, mapper-concerns, mapper-domain}]})`.
...
- **Steps**:
  ...
  2. Replace `STATE` variable with `FSTATE=".loop-spec/features/$SLUG/feature.json"`. Add the new `assert_file "$FSTATE"`.
...
- **Files**: `agents/loop-spec-*.md` (any files containing the banned keys)
- **Verify**: `bash -c '! grep -E "^(skills|mcpServers):" agents/loop-spec-*.md && bash tests/validate-agents.sh'`
...
- **Wave**: 8
- **Steps**:
  1. `grep -lE '^(skills|mcpServers):' agents/loop-spec-*.md` to identify offenders.
  2. For each offender, read the file, remove the offending key + value lines from the YAML frontmatter.
  3. If a removed `skills:` was load-bearing for the agent's behavior, document an inline `Skill(loop-spec:{name})` invocation in the agent's prompt body per SPEC breaking-change note. If any phase-skill rewrite (013-018) removed a Skill() invocation from an agent prompt body, document the inline invocation in the agent prompt instead.
...
- **Steps**:
  ...
  6. Confirm no `feature.json` is committed under `docs/loop-spec/features/cycle-agent-teams/`: `! test -f docs/loop-spec/features/cycle-agent-teams/feature.json`.
