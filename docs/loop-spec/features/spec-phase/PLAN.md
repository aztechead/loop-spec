...
5. The `loop-spec-spec-interviewer` agent is a non-restricted agent (has Write/Edit) and must NOT be added to `RESTRICTED_AGENTS` in `validate-agents.sh`. Its path restriction comes from the `hooks/restrict-agent-paths.sh` case block (task-03).
...
**read_first:**
- `docs/loop-spec/features/spec-phase/SPEC.md` (success criteria, decisions, user-facing behavior)
...
- `docs/loop-spec/features/spec-phase/PATTERNS.md` (Concept: skill SKILL.md structure)

**Steps:**

1. Write a failing verify: `test -f /Users/cbobrowitz/Projects/loop-spec/skills/spec/SKILL.md` -- confirm it fails (file absent).
...
3. Apply the skill structure pattern from `skills/discuss/SKILL.md:1-12` (frontmatter: `name: spec`, description, no tools list -- skills are markdown). Include:
   ...
   - Step 1: TeamCreate `loop-spec-spec-{slug}` with `spec-interviewer-1`, `advocate-1`, `challenger-1` (model from preset-matrix; see Assumption 3)
   ...
   - Step 5: Commit SPEC.md: `git add docs/loop-spec/features/{slug}/SPEC.md && git commit -m "spec: NO_JIRA {slug}"`
...
4. Add **Non-interactive mode** section documenting `LOOP_SPEC_NON_INTERACTIVE=1` behavior: the spec-interviewer agent replaces AskUserQuestion calls (round confirmations, round-6 override prompt) with env var reads. Document env vars: `LOOP_SPEC_ANSWER_SPEC_CONFIRM=yes` (skip "gate passed, proceed?" prompt), `LOOP_SPEC_ANSWER_SPEC_OVERRIDE=yes` (auto-override at round 6 if gate fails).
5. Run verify: `test -f /Users/cbobrowitz/Projects/loop-spec/skills/spec/SKILL.md` -- confirm exit 0.

**verifyCommand:** `test -f /Users/cbobrowitz/Projects/loop-spec/skills/spec/SKILL.md`
...
- Includes a Non-interactive mode section referencing `LOOP_SPEC_NON_INTERACTIVE=1`
...
### task-02: agents/loop-spec-spec-interviewer.md -- new agent definition

**Subject:** Create `agents/loop-spec-spec-interviewer.md` with all 5 perspectives, scoring rubric (verbatim formula), gate thresholds, and write contract.

**files:**
- `agents/loop-spec-spec-interviewer.md`
- `tests/validate-agents.sh`

**blockedBy:** []

**read_first:**
- `agents/loop-spec-spec-writer.md` (agent frontmatter pattern, write contract, report format)
- `agents/loop-spec-advocate.md` (minimal frontmatter reference)
- `docs/loop-spec/features/spec-phase/SPEC.md` (verbatim formula, gate values, perspective names)
...
- `docs/loop-spec/features/spec-phase/PATTERNS.md` (Concept: agent frontmatter schema)

**Steps:**

1. Write a failing verify: `test -f /Users/cbobrowitz/Projects/loop-spec/agents/loop-spec-spec-interviewer.md` -- confirm absent.
2. Apply the agent frontmatter pattern from `agents/loop-spec-spec-writer.md:1-11`:
   ```markdown
   ---
   name: loop-spec-spec-interviewer
...
3. Write the agent body:
   ...
   - Write-path constraint: `docs/loop-spec/features/{slug}/SPEC.md` and `.loop-spec/features/{slug}/spec-interview-transcript.md`
...
5. Confirm no em-dash in the new file: `grep -c -- "—" agents/loop-spec-spec-interviewer.md` returns 0.
6. Run verify: `grep -F "1.0 - (0.35 * goal_clarity + 0.25 * boundary_clarity + 0.20 * constraint_clarity + 0.20 * acceptance_clarity)" agents/loop-spec-spec-interviewer.md` exits 0.

**verifyCommand:** `grep -F "1.0 - (0.35 * goal_clarity + 0.25 * boundary_clarity + 0.20 * constraint_clarity + 0.20 * acceptance_clarity)" /Users/cbobrowitz/Projects/loop-spec/agents/loop-spec-spec-interviewer.md`

**acceptanceCriteria:**
- `agents/loop-spec-spec-interviewer.md` exists with correct frontmatter (`name:` matches filename, `description` non-empty, `tools` list present, `model` in allowed set)
...
- `grep -E "0\.20|0\.6|0\.5|0\.4" agents/loop-spec-spec-interviewer.md | wc -l` returns >= 4 (gate threshold and per-dimension minimums present)
...
**Subject:** Add `loop-spec-spec-interviewer` to the write-restriction case block in `hooks/restrict-agent-paths.sh` so it is restricted to `docs/loop-spec/features/**`.
...
- `docs/loop-spec/features/spec-phase/PATTERNS.md` (Concept: path-restriction hook registration)
- `docs/loop-spec/codebase/CONCERNS.md:25` (permissive default risk note)

**Steps:**

1. Write a failing verify: grep for `loop-spec-spec-interviewer` in `hooks/restrict-agent-paths.sh` -- confirm absent.
2. Apply the case-block pattern from `hooks/restrict-agent-paths.sh:86`:
   ...
   to:
   ```bash
   loop-spec-spec-writer|loop-spec-planner|loop-spec-pattern-mapper|loop-spec-spec-interviewer)
   ```
3. Run verify: `grep -q "loop-spec-spec-interviewer" /Users/cbobrowitz/Projects/loop-spec/hooks/restrict-agent-paths.sh && echo ok`
4. Run the existing hook test to confirm no regression: `bash /Users/cbobrowitz/Projects/loop-spec/hooks/restrict-agent-paths.test.sh`

**verifyCommand:** `grep -q "loop-spec-spec-interviewer" /Users/cbobrowitz/Projects/loop-spec/hooks/restrict-agent-paths.sh && bash /Users/cbobrowitz/Projects/loop-spec/hooks/restrict-agent-paths.test.sh`

**acceptanceCriteria:**
- `loop-spec-spec-interviewer` appears in the pipe-delimited case list at line 86 (or equivalent line after edit)
- `hooks/restrict-agent-paths.test.sh` exits 0 with no new failures
- The existing `loop-spec-spec-writer|loop-spec-planner|loop-spec-pattern-mapper` restriction is preserved (not split into a separate case block)
...
- `docs/loop-spec/features/spec-phase/SPEC.md` (decision: schemaVersion 4 fields list)
- `docs/loop-spec/features/spec-phase/PATTERNS.md` (Concept: feature.json schemaVersion bump)
...
7. In the `artifacts` object, add `"specInterview": "path or null (.loop-spec/features/{slug}/spec-interview-transcript.md)"` alongside `artifacts.spec`.
...
9. Run verify: `grep -E "schemaVersion.*4|spec.*perPhase|specInterview" /Users/cbobrowitz/Projects/loop-spec/skills/shared/feature-state-schema.md | wc -l` returns >= 3.

**verifyCommand:** `grep -E "schemaVersion.*4|spec.*perPhase|specInterview" /Users/cbobrowitz/Projects/loop-spec/skills/shared/feature-state-schema.md | wc -l`
...
- `docs/loop-spec/features/spec-phase/PATTERNS.md` (Concept: lib bash script with set -euo pipefail; Concept: test suite for a lib script)
- `docs/loop-spec/features/spec-phase/SPEC.md` (Exceptional criteria: idempotency, 3 test cases)
...
6. Run verify: `test -x /Users/cbobrowitz/Projects/loop-spec/lib/migrate-schema-v3-to-v4.sh`

**verifyCommand:** `test -x /Users/cbobrowitz/Projects/loop-spec/lib/migrate-schema-v3-to-v4.sh && bash /Users/cbobrowitz/Projects/loop-spec/tests/lib/migrate-schema-v3-to-v4.test.sh`
...
- `docs/loop-spec/features/spec-phase/PATTERNS.md` (Concept: cycle Step 5 initialization; Concept: cycle Step 6 routing; Concept: resume with schemaVersion detection)
...
3. **Step 6 description update**: edit the prose that describes the first phase from "discuss" to "spec". The routing logic itself (`Skill(loop-spec:{currentPhase})`) is already generic -- only the description text needs updating.
...
5. **Step 1 resume detection -- schemaVersion 3 check**: after successfully parsing feature.json and before the orphan probe, add:
   ...
   - If `schemaVersion == 3`: prompt the user via AskUserQuestion:
     ...
     - "Migrate to v4": run `bash "${CLAUDE_PLUGIN_ROOT}/lib/migrate-schema-v3-to-v4.sh" ".loop-spec/features/{slug}"`, reload feature.json
   ...
   ```
6. Run verify: `grep -E "schemaVersion.*4|currentPhase.*spec" /Users/cbobrowitz/Projects/loop-spec/skills/cycle/SKILL.md | wc -l` returns >= 2.
7. Run verify: `grep -E "spec.*first|route.*spec|Skill.*spec" /Users/cbobrowitz/Projects/loop-spec/skills/cycle/SKILL.md` exits 0.
8. Run verify: `grep -E "schemaVersion.*3.*migrate|AskUserQuestion.*migrate|migrate.*v3" /Users/cbobrowitz/Projects/loop-spec/skills/cycle/SKILL.md` exits 0.

**verifyCommand:** `grep -E "schemaVersion.*4|currentPhase.*spec" /Users/cbobrowitz/Projects/loop-spec/skills/cycle/SKILL.md | wc -l`
...
- Non-interactive default for migration is "Continue on v3" unless `LOOP_SPEC_ANSWER_MIGRATE_SCHEMA=1`
...
- `docs/loop-spec/features/spec-phase/SPEC.md` (Exceptional criterion for ambiguity_scores in discuss)
...
2. In the `body:` of the `SendMessage` call, add after "Produce SPEC.md at the output path per your role definition":
   ```
   If `docs/loop-spec/features/{slug}/SPEC.md` already exists and contains an `ambiguity_scores` frontmatter block (produced by the spec phase), treat the spec phase output as locked requirements. Read the `ambiguity_scores` block for context. Do NOT overwrite or remove the `ambiguity_scores` block when revising SPEC.md.
   ```
3. Run verify: `grep -i "ambiguity_scores" /Users/cbobrowitz/Projects/loop-spec/skills/discuss/SKILL.md` exits 0.

**verifyCommand:** `grep -i "ambiguity_scores" /Users/cbobrowitz/Projects/loop-spec/skills/discuss/SKILL.md`
...
- `docs/loop-spec/features/spec-phase/SPEC.md` (success criteria for README and CHANGELOG)

**Steps:**

1. Read `README.md` and `CHANGELOG.md` fully before editing.
2. In `README.md`:
   - Update the phase table (lines ~90-95) to prepend a SPEC row: `| **SPEC** | \`docs/loop-spec/features/{slug}/SPEC.md\` + \`ambiguity_scores\` frontmatter | 6-round Socratic interview, ambiguity gate |`
...
3. In `CHANGELOG.md`, add under `## [Unreleased]` a new `### Added` bullet (or add to existing Added section):
   ```markdown
   - **Spec phase** (`skills/spec/SKILL.md`): new SPEC -> DISCUSS -> PLAN -> EXECUTE -> VERIFY phase order. The spec phase runs a Socratic interview (up to 6 rounds, 5 rotating perspectives) with a quantitative ambiguity gate (ambiguity <= 0.20, per-dimension minimums). Produces SPEC.md with `ambiguity_scores` frontmatter. Agent: `agents/loop-spec-spec-interviewer.md`.
   ...
   ```
4. Run verify: `grep -E "SPEC.*DISCUSS.*PLAN|spec.*discuss.*plan" /Users/cbobrowitz/Projects/loop-spec/README.md` exits 0.
5. Run verify: `grep -A5 "Unreleased" /Users/cbobrowitz/Projects/loop-spec/CHANGELOG.md | grep -i "spec.*phase\|spec-phase\|schemaVersion"` exits 0.

**verifyCommand:** `grep -E "SPEC.*DISCUSS.*PLAN|spec.*discuss.*plan" /Users/cbobrowitz/Projects/loop-spec/README.md`
...
- `docs/loop-spec/features/spec-phase/PATTERNS.md` (Concept: test suite for a lib script -- run-all registration pattern)
...
2. Add a `run_suite` call for the new migration test, after `run_suite "lib/detect-test-cmd"`:
   ...
   ```
3. Run the full suite: `bash /Users/cbobrowitz/Projects/loop-spec/tests/run-all.sh` -- confirm exit 0 and all suites pass.
4. Run validate-agents: `bash /Users/cbobrowitz/Projects/loop-spec/tests/validate-agents.sh` -- confirm output is `All 13 agents validated.`
5. No-em-dash scan across all new and modified files:
   ```bash
   grep -rn -- "—" \
     /Users/cbobrowitz/Projects/loop-spec/skills/spec/ \
     /Users/cbobrowitz/Projects/loop-spec/agents/loop-spec-spec-interviewer.md \
     /Users/cbobrowitz/Projects/loop-spec/lib/migrate-schema-v3-to-v4.sh \
     /Users/cbobrowitz/Projects/loop-spec/tests/lib/migrate-schema-v3-to-v4.test.sh
   ```
   Confirm no matches (0 lines output).

**verifyCommand:** `bash /Users/cbobrowitz/Projects/loop-spec/tests/run-all.sh`
