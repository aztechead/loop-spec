...
For the agent continuing `feat/7.x.x` (work is on branch `claude/7-0-release-handoff-0biuqi`, to be merged into `feat/7.x.x`). **This checkpoint completes tasks 004 through 007; tasks 008–012 remain.**

## Completed work

The approved release design is in SPEC.md and PLAN.md. EXECUTE has twelve tasks; tasks 001–007 are complete, reviewed, verified, and recorded done in [the progress snapshot](handoff/tasks-progress.json):
...
2. Continue with task 008 in PLAN order; do not dispatch tasks 001–007 again. Each task's Verify command in [the progress snapshot](handoff/tasks-progress.json) is authoritative.
