...
| Deferral protection was vocabulary-only | Removing deferral wording after a Stop denial could pass without implementing the omitted scope. | The guard persists an obligation tied to transcript position and repository state, then requires post-denial work, later verification, and a `Resolved scope:` evidence line. |
