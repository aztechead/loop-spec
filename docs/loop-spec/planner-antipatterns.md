...
**GOOD:** `LOOP_SPEC_FOO=0 bash hooks/team/foo.sh <<< '{}'` exits 0 with no stdout output.
...
**GOOD:** `grep -c "^### task-[0-9]\+:" docs/loop-spec/features/foo/PLAN.md` returns 3 or more; `grep -c "read_first:" docs/loop-spec/features/foo/PLAN.md` returns 3 or more.
