...
document: M7 cannot remove a surface whose row is missing or still `open`.
...
| `removed` | The behavior goes away. Each removal cites the roadmap decision that covers it, or is listed under [Removals this inventory proposes](#removals-this-inventory-proposes). |
| `open` | No roadmap decision covers it and this document does not decide it. Listed under [Decisions this inventory asks for](#decisions-this-inventory-asks-for). |
...
| `auto` | removed | proposed; the caller names the entry, and the route judge is gone with oneshot |
| `revise` | open | PR feedback after human review. See decisions |
| `rollback` | removed | proposed; state is off-branch (R§12) and the program never resets a branch (R§7). Git history is the user's |
| `assess` | removed | proposed; outside the seven-phase scope (R§20) |
| `forensics` | removed | proposed; `status` plus `events.jsonl` in the state home cover the read side |
| `walkthrough` | removed | proposed; the PR body carries the rendered summary (R§12) |
| `quality-loop` | removed | proposed; review is a role inside EXECUTE and VERIFY |
| `checking-gates` | removed | proposed; user-gate tasks depend on the host task list, which 7.x does not use |
| `specifying-gates` | removed | proposed; same reason |
...
| `rules` | removed | proposed; self-learning `RULES.md` loop |
| `retro` | removed | proposed; outside scope |
| `sentinel` | removed | proposed; outside scope |
| `watch` | removed | proposed; outside scope |
| `loop-runner` | removed | proposed; the bundled loop runner and its offline suite |
...
| `challenger` | open | critique gate. See decisions |
...
| `critique-gate-protocol.md` | open | with `challenger` |
...
| `dual-process.md` | removed | proposed; effort routing |
...
| `prompt-normalize.md` | removed | proposed; request text is taken as given |
| `pr-feedback-check.md` | open | with `revise` |
...
| `laziness-ladder.md`, `human-code.md`, `human-docs.md`, `design-for-change.md` | open | contributor guidance for this repo; see the maintainer-tooling decision |
...
| `adopt-pr.sh` | open | adopting an existing PR as the execution branch. See decisions |
...
| `comment-tells.sh` | open | maintainer tooling; see decisions |
...
| `doc-deps.sh` | open | dependency-idiom grounding; see decisions |
| `doc-tells.sh` | open | maintainer tooling |
| `docs-probe.sh` | open | with `doc-deps.sh` |
| `duplication-scan.sh` | open | maintainer tooling |
| `effort-probe.sh` | removed | proposed; effort routing |
...
| `failure-tells.sh` | open | maintainer tooling |
...
| `owned-gitignore.sh` | open | with the `last-result.json` pointer decision |
...
| `regression-scan.sh` | removed | proposed; it reads committed VERIFICATION.md, which 7.x does not commit |
...
| `revise-branch.sh` | open | with `revise` |
| `revise-state.sh` | open | with `revise` |
...
| `runtime-ignore.sh` | open | with the `last-result.json` pointer decision |
| `runtime-preflight.sh` | replaced | program runtime check |
| `security-signal.sh` | open | see decisions |
...
| `surface.sh` | open | maintainer tooling |
...
| `trust.sh` | removed | proposed; outside scope |
| `tuning.sh` | removed | proposed; outside scope |
...
| `verify-live.sh` | removed | proposed; the live-run rung |
...
|---|---|---|
| `critique_prompt.py` | open | with `challenger` |
| `design_budget.py` | removed | with `design-budget.sh` |
| `diff-added-lines.py` | replaced | reviewed ranges in the ledger (R§10) |
| `doc-deps.py`, `docs-probe.py` | open | with `doc-deps.sh` |
| `doc-tells.py`, `failure-tells.py` | open | maintainer tooling |
...
| `surface.py` | open | maintainer tooling |
...
and 27 test files. R§16: the Claude Code plugin keeps one hook and 7.x depends on no
hook for correctness. Which hook is kept is an open decision.
...
| `team/done-criteria.sh` | UserPromptSubmit | removed | proposed |
...
| `team/session-end-learnings.sh` | Stop | removed | proposed; learnings JSONL |
...
| `hooks.json` | manifest | replaced | a `hooks.json` registering the one kept hook |
...
| `LAST_RESULT_FILE` | open | with the pointer decision |
...
| `NON_INTERACTIVE`, `AUTONOMOUS`, `PAUSE` | open | see decisions |
...
| `PR_FEEDBACK_MODE`, `PR_FEEDBACK_OWNER`, `REVIEW_BOT_ALLOWLIST`, `ISSUE_INTAKE_CLAUDE_FLAGS`, `MAX_FEATURES`, `ROLLBACK_CONFIRMED` | open | with `revise`, `issue-intake`, `rollback` |
| `CREDENTIAL_REFRESH_CMD`, `CREDENTIAL_REFRESH_TIMEOUT_SECONDS`, `CREDENTIAL_REFRESH_STAGE`, `CREDENTIAL_REFRESH_REPO`, `CREDENTIAL_REFRESH_REASON`, `CREDENTIAL_REFRESH_HOST`, `CREDENTIAL_PREPARED_STAGES`, `AUTH_ERROR_CODE`, `AUTH_ERROR_MESSAGE` | open | with `credential-refresh.sh` |
...
| `DOC_DEPS`, `DOCS_FIXTURES`, `DOCS_CACHE_TTL_SECS`, `DOCS_CACHE_DIR` | open | with `doc-deps.sh` |
| `INDIRECTION_MAX_BODY`, `DUP_MIN_LINES` | open | maintainer tooling |

New in 7.x, for completeness: `LOOP_SPEC_HOME` (R§12), `LOOP_SPEC_PHASE_<NAME>` and
`LOOP_SPEC_ROLE_<ROLE>` (R§5), and the `LOOP_SPEC_QUESTION` marker (R§15).
...
| `extensions/sessions/session_run.py`, `cycle_run.py`, `profiles/*.toml`, `NOTICE`, `README.md` | removed | proposed; the session layer served the dropped harnesses. The SDK runner is the unattended path (R§9) |
| `examples/supervisor/*` | replaced | the supervisor example that ships with 7.0 (R§19: the catch-all approval must not survive) |
| `examples/foreign-claimant/*` | removed | proposed; demonstrates `LOOP_SPEC_FOREIGN_CLAIMANTS`, which goes |
| `llms.txt` | replaced | rewritten for the 7.x surface |
| `README.md`, `CLAUDE.md`, `CHANGELOG.md` | replaced | rewritten at M7 (R§17, R§18) |
| `evals/` | removed | empty apart from a cache directory |
| `inbox/` | open | PR-108 and PR-94 working notes; see decisions |
...
| `docs/loop-spec/gdd.md` | open | with `greenfield-bootstrap.sh` |
...
| `docs/adopting.md`, `docs/tier-guide.md`, `docs/determinism-audit.md`, `docs/examples/` | replaced | rewritten or dropped at M7; tier guide goes with the tier matrix |
| `tests/` (62 top-level, 173 under `tests/lib/`) | removed | the 7.x offline suite (R§17) |

## Decisions this inventory asks for

Each item is `open` above. M7 cannot delete an `open` row.

1. `revise` and PR feedback. `revise`, `pr-comments.sh`, `pr-feedback.sh`,
   `revise-branch.sh`, `revise-state.sh`, `pr-feedback-check.md`, and their variables.
   Options: a `revise` entry that enters EXECUTE in `remediation` mode from PR
   comments, or removal with human review handled outside loop-spec.
2. The critique gate. `challenger`, `critique-gate-protocol.md`,
   `critique-step.sh`, `critique_prompt.py`. Options: an optional critic role
   bindable in SPEC and PLAN, or removal with ITERATE as the second opinion (R§6).
3. The one kept hook. R§16 keeps one Claude Code hook and does not name it.
4. Maintainer tooling. `comment-tells`, `doc-tells`, `failure-tells`,
   `house-style`, `indirection-scan`, `duplication-scan`, `surface`, and the four
   contributor contracts. Options: move to a `tools/` directory outside the shipped
   skill tree, or remove and rewrite `CLAUDE.md` without them.
5. Dependency-idiom grounding. `doc-deps`, `docs-probe`, and their variables.
   Options: fold into the `implementer` method as a program-supplied input, or remove.
6. Credential refresh. `credential-refresh.sh` and the `CREDENTIAL_*` and
   `AUTH_ERROR_*` variables. R§4 records an expired credential as an observed `blocked`
   cause. Options: no refresh, `delivery blocked` names the repair; or a config
   refresh command run once before DELIVER.
7. Unattended mode signal. `NON_INTERACTIVE`, `AUTONOMOUS`, `PAUSE`. The SDK
   runner is chosen explicitly (R§9); whether any environment flag still selects
   question policy is unspecified.
8. The `last-result.json` pointer. R§15 keeps `.loop-spec/last-result.json` in
   the consumer repo; R§12 commits nothing by default. `owned-gitignore.sh`,
   `runtime-ignore.sh`, and `LAST_RESULT_FILE` depend on the answer.
9. Adopting an existing PR (`adopt-pr.sh`) as the execution branch.
10. Greenfield mode (`greenfield-bootstrap.sh`, `docs/loop-spec/gdd.md`) for a
    repository with no manifests to baseline.
11. GitHub issue intake (`issue-intake.sh`) as an unattended entry.
12. The security signal probe (`security-signal.sh`). Options: a required input to
    the review contract, or removal with security folded into the reviewer's method.
13. `inbox/`: archive or delete the PR-94 and PR-108 working notes.

## Removals this inventory proposes

Removals no roadmap decision covers. Sign-off here makes them accepted removals.
