...
- Write/Edit calls made by write-scoped agents are gated by a `PreToolUse` hook at runtime. Spec-writer, planner, and pattern-mapper agents are restricted to `docs/loop-spec/features/**`. Mapper agents are restricted to `docs/loop-spec/codebase/**`. Violations exit 2, blocking the tool call.
