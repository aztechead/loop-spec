...
| **Skill** | A reusable, named procedure invoked via `Skill(loop-spec:{name})`. Skills are Claude Code plugin entry points defined in `skills/{name}/SKILL.md`. |
| **Agent** | A one-shot, specialized subagent dispatched via the `Agent` tool. Each agent has a fixed role, a tool allow-list, and a default model. Agents are defined in `agents/loop-spec-{role}.md`. |
...
| **Codebase Map** | Five committed markdown files (`TECH.md`, `ARCH.md`, `QUALITY.md`, `CONCERNS.md`, `DOMAIN.md`) under `docs/loop-spec/codebase/` that describe the consuming project. Generated once on first cycle run, refreshed incrementally at end of each cycle. |
...
| **Critique Gate** | A challenger-only review gate run during DISCUSS (on SPEC.md) and PLAN (on PLAN.md). The lead adjudicates findings into a fix-list. DISCUSS skips when the spec is already gated; PLAN skips on the structural fast-path. There is no advocate. |
...
| **GSD / get-shit-done** | An upstream multi-phase spec-driven workflow (`gsd-build/get-shit-done`). loop-spec can ingest its `.planning/codebase/` artifacts on first run via `lib/gsd-ingest.sh`. DOMAIN.md has no GSD analog and is always freshly mapped. |
...
| **TDD Ordering** | The requirement that code-producing tasks write a failing test before implementing (red then green), even when the plan omits a TDD label. Skill/config/docs tasks are excluded. Enforced by the planner's role definition and every implementer dispatch. |
| **Non-interactive Mode** | A CI/scripting mode activated by `LOOP_SPEC_NON_INTERACTIVE=1`. Reads tier, preset, style, and title from env vars, bypassing all `AskUserQuestion` calls. |
| **index.json** | A tracked file at `.loop-spec/codebase/index.json` mapping file paths to the codebase domains they belong to. Used to compute which domains are stale after a feature branch lands. |
...
Each domain is a file under `docs/loop-spec/codebase/` and is refreshed incrementally based on which files changed since the last cycle.
...
1. User invokes `Skill(loop-spec:cycle)`.
...
| **Git / GitHub** | Hosts the consuming project's repository. loop-spec creates branches, commits, worktrees, and PRs via git CLI and `gh`. |
...
| **Claude Code plugin marketplace** | The mechanism by which loop-spec is distributed and installed into a consuming project (`claude plugin install loop-spec@loop-spec-marketplace`). |
