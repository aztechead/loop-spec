...
| `LOOP_SPEC_DESIGN_BUDGET_MINS` | unset; route estimate | Cumulative soft allowance for SPEC and PLAN. Auto mode uses `10 + 2 × estimated files + criteria`, capped at 60 (7 files/5 criteria = 29 minutes); missing or malformed route evidence uses 60. Set a positive integer from 1 to 3600 to deliberately extend a resumed run. Handoff and resume accounting uses recorded phase elapsed telemetry plus the active phase clock; this is wall-clock accounting and may include time around handoffs. Entering or completing EXECUTE, ONESHOT, VERIFY, or ITERATE resets the design allowance for a later design rewind. Malformed telemetry is ignored safely. It never preempts a phase; unresolved required gate failures after exhaustion escalate. |
...
| `LOOP_SPEC_PLAN_MIN_WIDTH` | non-negative integer; `2` | The wave width a plan must reach at PLAN exit, measured over declared plus file-overlap edges (`lib/plan-exit-gate.sh`, `[width]`). By default the floor applies only to plans of four or more tasks; a non-empty override also applies it to smaller plans. `1` accepts a chain. An invalid value is a configuration error. |
| `LOOP_SPEC_REVIEW_GROUP_BYTES` | positive integer; `150000` | Byte cap on the review packages one spec-compliance reviewer reads per wave on the EXECUTE subagent rung (`lib/execute-step.sh review-groups`). A wave under the cap is one reviewer launch; a group never splits below one task. Any other value is a configuration error. |
| `LOOP_SPEC_ROUTE` | `full`; unset | `full` keeps a run off the oneshot route (`lib/graph/probes/oneshot.sh` answers `route=full`). The override lengthens only: any other value also answers `full`, because nothing demotes a full run to a oneshot. Set to anything, it also skips the SPEC-entry route judge: `cycle-driver.sh spec judge` answers `skipped` with no dispatch, since the probe would not read the verdict. |
...
| `LOOP_SPEC_SAME_SESSION` | `0`/`1`; `0` | `1` runs every phase in the one session that started the cycle: `cycle-driver.sh next` never answers `HANDOFF`, and `hooks/team/phase-handoff-guard.sh` stands down, as if every graph edge carried `sameSession`. Opt in for an operator who wants one context end to end. |
| `LOOP_SPEC_SESSION_ID` | non-empty stable id; adapter supplied | The canonical identity for one lead invocation. Adapters and the headless lead launcher supply it automatically; standalone driver callers must set it once and reuse it across tools. Start a fresh phase session with a new id. `LOOP_SPEC_SAME_SESSION=1` is the explicit exception for one-session runs. |
| `LOOP_SPEC_DISPATCH_WAIT_MINS` | integer; `90` | How long a fresh `.pending-dispatch` marker (written by the `dispatch` event, cleared by the next driver call) lets `hooks/team/cycle-stamp-guard.sh` allow a turn to end while an agent is still running. |
...
| `LOOP_SPEC_STARTUP_BASELINE` | `0`/`1`; `0` | `1` durably opts the first implementation entry (EXECUTE or ONESHOT) into an exact-base test/lint/typecheck baseline after design artifacts exist and before implementation writes. `phase-begin` may run preparation and this suite before returning the phase packet. A completed capture or handled failure sets `verificationBaselineAttempted`; an interrupted capture remains retryable against the exact base. In workspace mode each completed repository is persisted immediately. `LOOP_SPEC_WORKTREES=0` skips capture with a notice because exact-base capture requires a temporary worktree; unknown values are treated the same way, with the attempt left retryable. Default `0` skips that suite: `verificationBaseline` stays `null` and every repository-wide failure observed later in the cycle blocks. Greenfield never captures a baseline. Enable it only where the base commit is already red and the known-failure oracle is what keeps EXECUTE and VERIFY from chasing failures the feature did not cause. |
...
| `LOOP_SPEC_MAX_PARALLEL_IMPLEMENTERS` | positive integer; unset | Requested implementer wave cap. With neither this nor `LOOP_SPEC_MAX_PARALLEL_SUBAGENTS` set, the cap is the plan's DAG width up to `3` (`lib/execute-rung.sh`). The effective value is the lower of this setting and `LOOP_SPEC_MAX_PARALLEL_SUBAGENTS`; `LOOP_SPEC_WORKTREES=0` forces both values to `1`. Teams and Workflow remain disabled because their fan-out is not enforceable by this cap. |
| `LOOP_SPEC_MAX_PARALLEL_SUBAGENTS` | positive integer; `1` | Cap for loop-spec one-shot child dispatch. `1` is serial; a finite value above `1` enables bounded one-shot waves. Teams and Workflow remain disabled so every child honors the cap. |
...
| `LOOP_SPEC_EXECUTE_WORKFLOW` | `0`/`1`; `0` | Reserved for an unbounded Workflow policy; finite child bounds make this rung ineligible. |
| `LOOP_SPEC_PLAN_MULTI_ANGLE` | `0`/`1`; `0` | Reserved for an unbounded Workflow policy; finite child bounds make this rung ineligible. |
...
| `LOOP_SPEC_HARNESS` | `claude`/`opencode`/`adk`/`codex`; detected | Forces the host adapter. The OpenCode plugin, ADK bridge, and Codex installer/hooks normally set this themselves. An unknown value falls through to detection, which defaults to `claude`. |
...
| `LOOP_SPEC_ARTIFACTS_IN_PR` | `0`/`1`; `1` | `0` copies `docs/loop-spec/features/<slug>/` and feature state to the artifact store during candidate finalization, then restores that document directory to its base image so run documents do not enter the delivery PR diff. Checkpoint draft PRs (`lib/checkpoint-pr.sh`) honor it too: the PR is opened from a sibling ref `<branch>-checkpoint` whose tip restores the directory to the base image, while the feature branch keeps the documents for resume and is pushed unchanged. `result.json` records the head as `checkpointPrHead`; a host that finds the run's PR by `branch` must look there first. |
...
| `LOOP_SPEC_DELIVER_ACCEPT_REMOTE_PATHS` | colon-separated globs; unset | Final delivery only. When the PR branch on the remote holds a descendant of the verified SHA whose every changed path (both sides of a rename) matches a glob, for example a bot's `CHANGELOG.md` commit, delivery skips the push and binds the PR head to that commit instead of failing with `push_failed` or `pr_head_moved`. Nothing re-verifies the accepted commit, so list only files no test reads. A commit that lands after required checks pass is still refused. Each delivery target in `delivery.json` (and in `result.json`'s `delivery`) lists the accepted SHAs in `remoteHeadAccepted`. Do not fast-forward the local branch onto an accepted commit; that trips `post_gate_drift`. A later CI remediation must merge or rebase onto it before its next push. Unset keeps every comparison strict. |
...
| `LOOP_SPEC_REVIEW_BOT_ALLOWLIST` | comma-separated GitHub logins; empty | Forces `/loop-spec:revise` to keep issue comments from those bot logins. Does not unskip a self `<!-- loop-spec:revise -->` comment or a bare LGTM/Approved body. A REVIEW with `CHANGES_REQUESTED` and every inline `review_comment` are already kept for any bot (including GitHub's code-review agent); this list is only for bot *issue* comments that would otherwise be skipped as chatter. |
...
| `LOOP_SPEC_ANSWER_ITERATE_SPEC` | `reopen`/`ship`; `reopen` | On a non-interactive SPEC-level iteration gap, `reopen` returns to SPEC refinement; `ship` advances to DELIVER and records the accepted gap. |
...
| `LOOP_SPEC_INVOCATION_STAMP` | `1` | The UserPromptSubmit hook stamps the raw arguments of a `/loop-spec:<skill>` prompt to `.loop-spec/invocation-stamp.json`; `cycle-driver.sh start` restores any token the skill's prose rewrite dropped and consumes the stamp. `hooks/team/cycle-stamp-guard.sh` (Stop) denies while a `skill: cycle` stamp is still present and no newer `.loop-spec/last-result.json` exists, until `LOOP_SPEC_STAMP_MAX_AGE_MIN`; the same hook denies while a feature's newest `phase_start` has no `phase_end` and no newer `<feature_dir>/result.json`, until `LOOP_SPEC_PHASE_TIMEOUT_MINS`; a `phase_start` stamped with another session's id is that session's to close and never denies this one. `0` disables the stamp and with it the first deny. |
...
`spec`, `plan`, `execute`, `verify`, `iterate`, and `deliver` are cycle
...
| `LOOP_SPEC_IDENTITY_INPUT`, `LOOP_SPEC_HOOK_INPUT` | Internal JSON transport values used by the shared session-identity resolver and Claude shell environment adapter. |
...
PLAN, after the SPEC interview and its design-lock questions, with the source
