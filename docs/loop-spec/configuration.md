...
| `LOOP_SPEC_PLAN_MIN_WIDTH` | non-negative integer; `2` | The wave width a plan of four or more tasks must reach at PLAN exit, measured over declared plus file-overlap edges (`lib/plan-exit-gate.sh`, `[width]`). `1` accepts a chain. Any other value is a configuration error. |
