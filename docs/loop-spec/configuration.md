...
| `LOOP_SPEC_ARTIFACTS_IN_PR` | `0`/`1`; `1` | `0` copies `docs/loop-spec/features/<slug>/` and feature state to the artifact store during candidate finalization, then restores that document directory to its base image so run documents do not enter the delivery PR diff. Checkpoint draft PRs (`lib/checkpoint-pr.sh`) honor it too: the PR is opened from a sibling ref `<branch>-checkpoint` whose tip restores the directory to the base image, while the feature branch keeps the documents for resume and is pushed unchanged. |
