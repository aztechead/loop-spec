...
| `LOOP_SPEC_SECRET_GUARD` | `1` | `0` disables `hooks/team/secret-guard.sh`, the PreToolUse hook that denies a Write/Edit/MultiEdit under `docs/loop-spec/features/**` or `.loop-spec/features/**` whose new text carries an email address or a credential path, and a Bash command that reads a known credential file (`application_default_credentials.json`, `~/.aws/credentials`, `~/.netrc`, `~/.ssh/id_*`, `*.pem`, `*.p12`, `*.key`). Existence checks (`test -f`, `ls`) and the tools' own auth commands pass. |
