...
| `LOOP_SPEC_CYCLE_STAMP_GUARD` | `1` | Blocks stopping a session that was invoked as `/loop-spec:cycle` and never called the driver: `.loop-spec/invocation-stamp.json` with `skill: cycle` is still present (`cycle-driver.sh start` consumes it) and no `.loop-spec/last-result.json` is newer than it. Stands down past `LOOP_SPEC_STAMP_MAX_AGE_MIN`. |
