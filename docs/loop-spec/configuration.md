...
| `LOOP_SPEC_MAX_PARALLEL_IMPLEMENTERS` | positive integer; inherits the subagent cap (`1` by default) | Requested implementer wave cap. The effective value is the lower of this setting and `LOOP_SPEC_MAX_PARALLEL_SUBAGENTS`; `LOOP_SPEC_WORKTREES=0` forces `1`. Wider waves require lead-created task worktrees (`subagentIsolation=lead-worktree`). |
| `LOOP_SPEC_MAX_PARALLEL_SUBAGENTS` | positive integer; `1` | Cap for loop-spec one-shot child dispatch. `1` is serial and disables implicit teams, Workflow fan-out, and automatic loop fleets. Set a value above `1` explicitly to opt into wider dispatch; Workflow implementations retain their own internal fan-out limits. |
