...
| `LOOP_SPEC_STARTUP_BASELINE` | `0`/`1`; `0` | `1` captures the exact-base test/lint/typecheck baseline once, at the first implementation entry (EXECUTE or ONESHOT), after design artifacts exist and before implementation writes. Default `0` skips that suite run entirely: `verificationBaseline` stays `null` and every repository-wide failure observed later in the cycle blocks. Enable it only where the base commit is already red and the known-failure oracle is what keeps EXECUTE and VERIFY from chasing failures the feature did not cause. Greenfield never captures a baseline regardless. A failed opt-in capture is reported with a null baseline and is not retried on re-entry. |
