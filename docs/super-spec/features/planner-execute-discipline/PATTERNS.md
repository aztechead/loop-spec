...
> Produced by `super-spec-planner`. Read by `super-spec-planner` before drafting tasks.
...
**Why this analog:** It is the most recent Cycle 2 hook addition that demonstrates all three required properties: fail-open (exit 0 on any parse error), a named kill-switch env var (`SUPER_SPEC_USERGATE_GUARD=0`), and a trace-log append via a configurable path env var.
...
# Kill switch
[[ "${SUPER_SPEC_USERGATE_GUARD:-1}" == "0" ]] && exit 0
...
check "kill-switch: SUPER_SPEC_USERGATE_GUARD=0 exits 0" 0 \
  "$(payload_completed)" "" "SUPER_SPEC_USERGATE_GUARD=0"
...
- Kill-switch env var name must follow the `SUPER_SPEC_<FEATURE>=0` pattern; do not invent a different disabling value.
...
STATE_FILE="/tmp/super-spec-failures-${SESSION}.json"
...
bash "${CLAUDE_PLUGIN_ROOT}/lib/decision-coverage.sh" \
  "{spec_path}" "docs/super-spec/features/{slug}/PLAN.md"
exit_code=$?
```
...
- `docs/super-spec/planner-antipatterns.md` -- there is no existing reference doc of this type in the codebase. Structure it after `docs/design.md` (plain markdown, no frontmatter, developer-for-developer tone per CLAUDE.md).
