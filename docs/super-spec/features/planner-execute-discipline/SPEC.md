...
super-spec's planner and EXECUTE phase have six concrete failure modes that let bad plans and incomplete runs pass undetected:
...
<decisions>
- Decision: banned phrases list lives in `agents/super-spec-planner.md` (not a separate linting script), enforced by prose instruction.
...
- Decision: budget-gate hook reads cost from `metrics-session.json` if present, falls back to `SUPER_SPEC_CURRENT_COST_USD` env var; if `SUPER_SPEC_MAX_COST_USD` is unset the hook exits 0 unconditionally.
- Decision: `agents/super-spec-spec-writer.md` is updated to require a `<decisions>` block in SPEC.md output (currently optional). No other SPEC.md sections are added or restructured.
...
- All new hooks are opt-in via kill-switch environment variables (`SUPER_SPEC_STRATEGY_ROTATION=0`, `SUPER_SPEC_BUDGET_GUARD=0`).
...
An anti-patterns reference doc at `docs/super-spec/planner-antipatterns.md` shows six do/don't pairs that illustrate the difference between banned and required forms.
...
**Strategy-rotation hook.** A new `hooks/team/strategy-rotation.sh` fires on `PostToolUse` for Bash, Edit, and Write tools. It tracks consecutive tool failures per tool name in `/tmp/super-spec-failures-${SESSION:-default}.json`. When consecutive failures reach the threshold (default 2, overridden by `SUPER_SPEC_STRATEGY_ROTATION_THRESHOLD`), the hook emits a `hookSpecificOutput.additionalContext` block instructing the agent to stop, verbalize what the failure mode was, describe a completely different approach, and explain why the new approach avoids the same failure. The counter resets on any successful tool call. Setting `SUPER_SPEC_STRATEGY_ROTATION=0` disables the hook.

**Budget-gate hook.** A new `hooks/team/budget-gate.sh` fires on `PreToolUse` for Agent tool calls. It reads current session cost from `metrics-session.json` if present, otherwise from the `SUPER_SPEC_CURRENT_COST_USD` env var. If `SUPER_SPEC_MAX_COST_USD` is unset, the hook exits 0 unconditionally. At 80-99% of the ceiling it emits a `hookSpecificOutput.additionalContext` warning. At 100% or above it exits 2, blocking the Agent call. Setting `SUPER_SPEC_BUDGET_GUARD=0` disables the hook.
...
- [ ] `agents/super-spec-planner.md` contains a `read_first[]` requirement per task with language specifying it must contain concrete file identifiers.
  Verify: `grep -c "read_first" agents/super-spec-planner.md` returns 1 or more.

- [ ] `agents/super-spec-planner.md` contains a banned-phrases section with the phrase "BANNED" or "MUST NOT" near a list that includes "looks correct" and "properly configured".
  Verify: `grep -A 10 "BANNED\|MUST NOT" agents/super-spec-planner.md | grep -c "looks correct\|properly configured"` returns 1 or more.

- [ ] `docs/super-spec/planner-antipatterns.md` exists with at least one do/don't pair.
  Verify: `grep -c "DO NOT\|BAD\|GOOD\|Instead\|instead" docs/super-spec/planner-antipatterns.md` returns 1 or more.
...
- [ ] `agents/super-spec-spec-writer.md` requires a `<decisions>` block in the SPEC.md output (verify grep confirms a `<decisions>` or `decisions block` requirement in the agent prompt).
  Verify: `grep -c "<decisions>" agents/super-spec-spec-writer.md` returns 1 or more.
...
- [ ] `hooks/team/strategy-rotation.sh` resets the failure counter on a successful tool call and honors `SUPER_SPEC_STRATEGY_ROTATION=0` kill switch.
...
- [ ] `hooks/team/budget-gate.sh` emits a warning additionalContext when cost is between 80% and 99% of `SUPER_SPEC_MAX_COST_USD`.
  Verify: `bash hooks/team/budget-gate.test.sh` passes the "warn at 80%" case.

- [ ] `hooks/team/budget-gate.sh` exits 2 when cost is at or above 100% of `SUPER_SPEC_MAX_COST_USD`.
  Verify: `bash hooks/team/budget-gate.test.sh` passes the "block at 100%" case.

- [ ] `hooks/team/budget-gate.sh` exits 0 when `SUPER_SPEC_MAX_COST_USD` is unset and when `SUPER_SPEC_BUDGET_GUARD=0`.
...
- [ ] No em-dash character (U+2014) appears in any new or modified file.
  Verify: `grep -rP "—" agents/super-spec-planner.md docs/super-spec/planner-antipatterns.md skills/shared/artifact-templates/PLAN.md.template lib/decision-coverage.sh lib/plan-adherence.sh lib/detect-test-cmd.sh hooks/team/strategy-rotation.sh hooks/team/budget-gate.sh hooks/hooks.json agents/super-spec-spec-writer.md skills/plan/SKILL.md skills/execute/SKILL.md CHANGELOG.md` returns no matches.
