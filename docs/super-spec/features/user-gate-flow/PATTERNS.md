...
# Kill-switch: set SUPER_SPEC_SOME_GUARD=0 to disable
if [[ "${SUPER_SPEC_SOME_GUARD:-1}" == "0" ]]; then
  exit 0
fi
...
- Kill-switch env var name must be unique per hook; do NOT reuse `SUPER_SPEC_FEATURE_DIR` as a kill-switch.
...
- The reference skills reference `.tasks.json` (a superpowers-specific artifact). In super-spec, task state lives in the harness task list (`TaskGet`/`TaskUpdate`), not a `.tasks.json` file. Replace all `.tasks.json` references with `TaskGet`/`TaskUpdate` harness calls.
- The reference skills reference `/gate-check` and `/specify-gate` slash commands that do not exist in super-spec. Substitute the super-spec invoke pattern: `Skill(super-spec:checking-gates)` and `Skill(super-spec:specifying-gates)`.
- "Sync `.tasks.json`" in specifying-gates Step 3 becomes `TaskUpdate` with the full new description in super-spec.
