...
- The SPEC says "at least 3 agents gain an `effort:` field and at least 1 gains `disallowedTools:`"; the plan adds `effort:` to `super-spec-implementer`, `super-spec-verifier`, and `super-spec-spec-writer`, and `disallowedTools:` to `super-spec-implementer` (excluding push/PR tools). `super-spec-implementer` also gains `isolation: worktree`.
...
- Create: `skills/pause/SKILL.md` - /super-spec:pause skill documenting HANDOFF.json + .continue-here.md generation
- Create: `skills/forensics/SKILL.md` - /super-spec:forensics read-only 7-pattern diagnostic
- Create: `skills/rollback/SKILL.md` - /super-spec:rollback checkpoint rollback skill
- Create: `skills/discipline/SKILL.md` - /super-spec:discipline on|off|status skill
...
- Modify: `agents/super-spec-spec-writer.md` - require Boundaries + 2-tier Success criteria sections
- Modify: `agents/super-spec-implementer.md` - add isolation, effort, disallowedTools to frontmatter
- Modify: `agents/super-spec-verifier.md` - add effort field to frontmatter
- Modify: `agents/super-spec-planner.md` - add effort field to frontmatter
...
| task-010 | agents/super-spec-spec-writer.md + SPEC.md.template: Boundaries + 2-tier success | - | `agents/super-spec-spec-writer.md`, `skills/shared/artifact-templates/SPEC.md.template` | small |
| task-011 | Agent frontmatter: effort, disallowedTools, isolation + agents/README.md | - | `agents/super-spec-implementer.md`, `agents/super-spec-verifier.md`, `agents/super-spec-planner.md`, `agents/README.md` | small |
...
**Goal:** Create an executable bash lib script that generates two crash-recovery artifacts into `.super-spec/features/{slug}/`.
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (Pause and crash recovery section)
...
- [ ] `--dry-run` mode accepts a `--feature-dir <path>` argument (defaults to scanning `.super-spec/features/*/feature.json`).
...
- [ ] Both artifacts are written to `.super-spec/features/{slug}/` (not to project root).
- [ ] Kill switch: `SUPER_SPEC_PAUSE=0` causes exit 0 without writing files.
...
**Goal:** Create a new skill that documents the /super-spec:pause command and delegates to lib/pause-snapshot.sh.
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (Regression gate section)
...
- [ ] Reads `docs/super-spec/features/*/VERIFICATION.md` for each completed feature (skips the current feature by checking feature.json `currentPhase != completed` or by accepting a `--exclude <slug>` flag).
...
- [ ] Step 3: Implement `lib/regression-scan.sh` using `find docs/super-spec/features -name VERIFICATION.md` to locate targets. Parse verify commands via `grep` patterns. Use python3 inline for JSON construction (per PATTERNS.md python3 inline pattern from `hooks/team/task-completed.sh:39-47`).
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (Regression gate + Ralph executor sections)
...
- [ ] The HARD-GATE remediation section documents: if `pendingRemediationTasks.length <= SUPER_SPEC_RALPH_THRESHOLD` (default 3), invoke `lib/ralph-remediation.sh <feature-dir>`; else use the existing full EXECUTE team path.
- [ ] `SUPER_SPEC_RALPH_THRESHOLD` env var is documented with default value of 3.
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (Checkpoint rollback section)

**Verify:** `test -x lib/checkpoint.sh && bash lib/checkpoint.sh 2>&1 | grep -c "tag\|rollback" | grep -qE '^[1-9]' && echo PASS`

**Acceptance criteria:**
- [ ] Script is executable with `set -euo pipefail`.
- [ ] `tag <type>` subcommand: creates a git tag `super-spec-checkpoint-{type}-YYYYMMDD-HHMMSS` using `git tag`. Supports the 4 automatic types: `post-discuss`, `post-plan`, `post-execute`, `post-verify`, `pre-rollback`, `manual`.
- [ ] `rollback <tag>` subcommand: runs `git checkout <TAG> -- .` (NOT `git reset --hard`), then creates a new commit with message `chore: NO_JIRA rollback to <tag>`. Requires the caller to set `SUPER_SPEC_ROLLBACK_CONFIRMED=1` env var (instead of interactive confirmation which is not available in skill context).
...
- [ ] Step 2: Run test, expect FAIL.
- [ ] Step 3: Implement `lib/checkpoint.sh` following lib/git-ops.sh conventions (`set -euo pipefail`, function-based structure). Use `git tag "super-spec-checkpoint-${type}-$(date +%Y%m%d-%H%M%S)"` for tagging. Use `git checkout "$tag" -- .` for rollback.
- [ ] Step 4: Run test, expect PASS.
...
**Goal:** Create a new skill that documents the /super-spec:rollback command, all 6 checkpoint types, and the ROLLBACK confirmation requirement.
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (Ralph remediation executor section)
- `docs/super-spec/features/resilience-ops/PATTERNS.md` (Bash remediation loop concept - ralph.sh:84-108)
...
- [ ] Exits 0 immediately if task count exceeds `SUPER_SPEC_RALPH_THRESHOLD` (default 3); caller routes to full EXECUTE team.
...
- [ ] Step 1: Write test asserting `SUPER_SPEC_RALPH_THRESHOLD=3` + feature-dir with 4 pending tasks exits 0 immediately (above threshold). Also assert the script is executable.
- [ ] Step 2: Run test, expect FAIL.
...
**Goal:** Create a new skill that documents the /super-spec:forensics read-only diagnostic, all 7 anomaly patterns, and the report path format.
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (Forensics diagnostic section)
...
- [ ] Documents that the report is written to `.super-spec/forensics/report-{ISO-8601}.md` and ONLY to that path.
...
### task-010: agents/super-spec-spec-writer.md + SPEC.md.template - Boundaries + 2-tier success
...
**read_first:**
- `agents/super-spec-spec-writer.md` (full current content)
- `skills/shared/artifact-templates/SPEC.md.template` (full current content)
- `docs/super-spec/features/resilience-ops/SPEC.md` (Intent contract section)

**Verify:** `grep -c "Boundaries\|Good Enough\|Exceptional" agents/super-spec-spec-writer.md | grep -qE '^[3-9]|^[1-9][0-9]' && grep -c "Boundaries\|Good Enough\|Exceptional" skills/shared/artifact-templates/SPEC.md.template | grep -qE '^[3-9]|^[1-9][0-9]' && echo PASS`

**Acceptance criteria:**
- [ ] `agents/super-spec-spec-writer.md` mentions `## Boundaries (what NOT to do)` as a required section.
- [ ] `agents/super-spec-spec-writer.md` mentions `## Success criteria` with `### Good Enough` and `### Exceptional` as required subsections.
...
- [ ] Step 2: Edit `agents/super-spec-spec-writer.md`: in the "Role boundary" section, add bullet requiring Boundaries section; in the "What NOT to do" section, add a bullet prohibiting omission of Success criteria tiers.
...
**Files:**
- `agents/super-spec-implementer.md`
- `agents/super-spec-verifier.md`
- `agents/super-spec-planner.md`
- `agents/README.md`

**read_first:**
- `agents/super-spec-implementer.md` (full current frontmatter)
- `agents/super-spec-verifier.md` (frontmatter)
- `agents/super-spec-planner.md` (frontmatter)
- `tests/validate-agents.sh` (validate field constraints so additions do not break the test)
- `docs/super-spec/features/resilience-ops/SPEC.md` (Agent frontmatter section)

**Verify:** `grep -rl "effort:" agents/ | wc -l | tr -d ' ' | grep -qE '^[3-9]|^[1-9][0-9]' && grep -rl "disallowedTools:" agents/ | wc -l | tr -d ' ' | grep -qE '^[1-9]' && grep -c "isolation: worktree" agents/super-spec-implementer.md | grep -qE '^[1-9]' && test -f agents/README.md && echo PASS`

**Acceptance criteria:**
- [ ] `agents/super-spec-implementer.md` frontmatter gains `isolation: worktree`, `effort: high`, and `disallowedTools:` list (at minimum: `Push`, `CreatePullRequest`, or their equivalents to match the implementer's documented constraints).
- [ ] `agents/super-spec-verifier.md` frontmatter gains `effort: medium`.
- [ ] `agents/super-spec-planner.md` frontmatter gains `effort: medium`.
...
- [ ] Step 2: Edit `agents/super-spec-implementer.md` frontmatter: add `isolation: worktree`, `effort: high`, `disallowedTools: [Push]` (use whatever tool name the CC harness recognizes for git push; document the assumption).
- [ ] Step 3: Edit `agents/super-spec-verifier.md` frontmatter: add `effort: medium`.
- [ ] Step 4: Edit `agents/super-spec-planner.md` frontmatter: add `effort: medium`.
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (Discipline-mode section)
- `docs/super-spec/features/resilience-ops/PATTERNS.md` (Conf-file-driven feature toggle concept)

**Verify:** `bash hooks/team/discipline-inject.test.sh`

**Acceptance criteria:**
- [ ] `hooks/team/discipline-inject.sh` is executable with `set -euo pipefail`.
- [ ] Reads `.super-spec/discipline.conf` in `CLAUDE_PROJECT_DIR` (or CWD if env var absent); if file absent or `ENABLED=1` not present, exits 0 with `{}` (no injection).
...
- [ ] Kill switch: `SUPER_SPEC_DISCIPLINE=0` exits 0 with `{}` before reading conf file.
- [ ] Fail-open: `trap 'exit 0' ERR`.
- [ ] No emoji in the injected directive text.
- [ ] `discipline-inject.test.sh` has at least 3 test cases: "enabled inject" (conf file with ENABLED=1 produces additionalContext), "kill switch" (SUPER_SPEC_DISCIPLINE=0 exits 0 with no context), "file absent" (no conf file exits 0 with no context).
- [ ] `skills/discipline/SKILL.md` exists with frontmatter and documents `on`, `off`, and `status` subcommands that write/update `.super-spec/discipline.conf`.
...
- [ ] Step 2: Implement `hooks/team/discipline-inject.sh` using the kill-switch + fail-open pattern from `hooks/team/stop-deflection-guard.sh:27-43` and conf-file check from the octopus reference. Use `SUPER_SPEC_DISCIPLINE` as the kill switch var. Read conf from `${CLAUDE_PROJECT_DIR:-.}/.super-spec/discipline.conf`. Escape the directive for JSON using `printf '%s' "$DIRECTIVE" | sed 's/\\/\\\\/g; s/"/\\"/g' | tr '\n' ' '`.
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (Output compressor section)
- `docs/super-spec/features/resilience-ops/PATTERNS.md` (Per-session temp-file state concept + additionalContext injection concept)
...
- [ ] Kill switch: `SUPER_SPEC_COMPRESSOR=0` exits 0 immediately.
- [ ] Fail-open: `trap 'exit 0' ERR`.
- [ ] Threshold: 3000 chars (hard-coded constant, not configurable per SPEC out-of-scope).
- [ ] Debounce: fires on every 3rd qualifying call. State persisted in `${TMPDIR:-/tmp}/super-spec-compress-${SESSION}.count` where `SESSION="${CLAUDE_CODE_SESSION_ID:-${CLAUDE_SESSION_ID:-$$}}"`.
- [ ] Shape detection:
...
- [ ] No emoji in output.
- [ ] `output-compressor.test.sh` covers: "threshold trigger" (output >= 3000 chars compresses), "JSON array" (correct shape), "kill switch" (SUPER_SPEC_COMPRESSOR=0 exits 0 silently), "fail-open" (malformed JSON exits 0).
...
- [ ] Step 2: Implement `hooks/team/output-compressor.sh` adapting the octopus reference. Replace `OCTOPUS_COMPRESS_ENABLED` with `SUPER_SPEC_COMPRESSOR`. Use `${TMPDIR:-/tmp}` (not `/tmp`). Remove analytics file write. Remove emoji from summary string. Match the 3000-char threshold exactly.
- [ ] Step 3: Run test suite, expect PASS.
...
- `hooks/team/stop-deflection-guard.sh:27-43` (kill-switch + fail-open pattern)
- `docs/super-spec/features/resilience-ops/SPEC.md` (Done-criteria detector section)
- `docs/super-spec/features/resilience-ops/PATTERNS.md` (Compound task detection heuristics concept)

**Verify:** `bash hooks/team/done-criteria.test.sh`

**Acceptance criteria:**
- [ ] `hooks/team/done-criteria.sh` is executable with `set -euo pipefail`.
- [ ] Kill switch: `SUPER_SPEC_DONE_CRITERIA=0` exits 0 immediately.
- [ ] Fail-open: `trap 'exit 0' ERR`.
...
- [ ] Step 2: Implement `hooks/team/done-criteria.sh` adapting the octopus reference. Replace `OCTO_DONE_CRITERIA` kill switch with `SUPER_SPEC_DONE_CRITERIA`. Replace emoji in directive with plain text. Use `python3` for prompt extraction, falling back to grep. Match the three heuristics exactly as specified.
- [ ] Step 3: Run test suite, expect PASS.
- [ ] Step 4: Commit both files.
...
- `hooks/team/stop-deflection-guard.sh:27-43` (kill-switch + fail-open pattern)
- `docs/super-spec/features/resilience-ops/SPEC.md` (Session-end learnings section)
- `docs/super-spec/features/resilience-ops/PATTERNS.md` (JSONL append with FIFO cap concept)
...
- [ ] Kill switch: `SUPER_SPEC_LEARNINGS=0` exits 0 immediately.
- [ ] Fail-open: `trap 'exit 0' ERR`.
- [ ] Appends a JSONL line to `.super-spec/learnings.jsonl` (relative to `CLAUDE_PROJECT_DIR` or CWD). Line schema: `{"timestamp":"...","sessionId":"...","taskType":"...","approach":"...","outcome":"...","lesson":"..."}`.
...
- [ ] Creates `.super-spec/` directory if absent.
- [ ] `session-end-learnings.test.sh` covers: "append" (valid JSONL line appended), "cap at 50" (file trimmed to 50 lines when over 50), "kill switch" (SUPER_SPEC_LEARNINGS=0 exits 0, file untouched).
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (hooks.json wiring success criterion)
...
- `docs/super-spec/features/resilience-ops/SPEC.md` (all 11 success criteria for changelog text)
...
Each task produces a discrete commit. If VERIFY fails, `git log --oneline` to identify the offending commit(s) and `git revert <sha>` each one. The checkpoint tagging added in task-005/task-008 provides a `super-spec-checkpoint-post-plan-*` tag to roll back to if needed.
