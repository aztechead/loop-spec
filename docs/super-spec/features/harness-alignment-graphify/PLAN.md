...
**Spec:** `docs/super-spec/features/harness-alignment-graphify/SPEC.md`
**Patterns:** `docs/super-spec/features/harness-alignment-graphify/PATTERNS.md`
**Created:** 2026-05-28
...
5. The `restrict-agent-paths.sh` hook uses a `super-spec-mapper-*` glob (line 93) that covers the remaining three mappers after deletion of mapper-arch and mapper-tech; no update to that hook is needed.
...
- Delete: `agents/super-spec-mapper-arch.md` - superseded by graphify in graphify-present path
- Delete: `agents/super-spec-mapper-tech.md` - superseded by graphify in graphify-present path
...
- Modify: `agents/super-spec-planner.md` - add graphify query/path/explain preference
- Modify: `agents/super-spec-pattern-mapper.md` - add graphify query/path/explain preference
...
| task-005 | Delete mapper-arch and mapper-tech agents; update validate-agents.sh count to 12 | - | `agents/super-spec-mapper-arch.md`, `agents/super-spec-mapper-tech.md`, `tests/validate-agents.sh` | small |
...
| task-008 | Add graphify query/path/explain preference to planner and pattern-mapper agents | - | `agents/super-spec-planner.md`, `agents/super-spec-pattern-mapper.md` | small |
...
**read_first:**
- `/Users/cbobrowitz/Projects/super-spec/.claude/` (confirm only `worktrees/` is present, no existing `settings.json`)
- `PATTERNS.md` concept: "Claude Code settings.json key injection"

**Verify:** `cat /Users/cbobrowitz/Projects/super-spec/.claude/settings.json | python3 -c "import json,sys; d=json.load(sys.stdin); assert d['worktree']['baseRef'] == 'head', 'FAIL'"` -> exits 0 with no output.
...
- [ ] Step 1: Confirm `.claude/` directory exists and `settings.json` does not (`ls /Users/cbobrowitz/Projects/super-spec/.claude/`).
...
**Goal:** Remove `agents/super-spec-mapper-arch.md` and `agents/super-spec-mapper-tech.md` (superseded by graphify in the graphify-present path). Update the hard-coded `EXPECTED` constant in `tests/validate-agents.sh` from 14 to 12 in the same commit so the validator never sees a transient count mismatch.

**Files:**
- `agents/super-spec-mapper-arch.md` (delete)
- `agents/super-spec-mapper-tech.md` (delete)
...
- `agents/super-spec-mapper-arch.md` (confirm it exists; already read)
- `agents/super-spec-mapper-tech.md` (confirm it exists; already read)
- `PATTERNS.md` concept: "mapper agent deletion + validate-agents.sh count update"

**Verify:** `bash tests/validate-agents.sh` exits 0 and prints `All 12 agents validated.`. `test ! -f agents/super-spec-mapper-arch.md && test ! -f agents/super-spec-mapper-tech.md` exits 0.

**Acceptance criteria:**
- [ ] `agents/super-spec-mapper-arch.md` does not exist (`test ! -f` exits 0).
- [ ] `agents/super-spec-mapper-tech.md` does not exist (`test ! -f` exits 0).
...
- [ ] Step 1: Confirm file count before deletion: `ls agents/super-spec-*.md | wc -l` should return 14.
- [ ] Step 2: Delete `agents/super-spec-mapper-arch.md` and `agents/super-spec-mapper-tech.md`.
...
- [ ] Step 5: Run `test ! -f agents/super-spec-mapper-arch.md && test ! -f agents/super-spec-mapper-tech.md`; confirm exit 0.
...
**Goal:** Add a conditional `graphify . --update` call to `skills/verify/SKILL.md` Step 7 (map-codebase refresh), before the `Skill(super-spec:map-codebase)` invocation. The call is conditional on `command -v graphify`; it runs without `--wiki` (faster, no wiki rebuild needed at verify time).
...
- [ ] The graphify block appears in Step 7 (map-codebase refresh), before the `Skill(super-spec:map-codebase)` invocation line.
...
- [ ] Step 2: Insert a conditional graphify block at the start of Step 7, before the `Skill(super-spec:map-codebase)` line. Pattern: `if command -v graphify: graphify . --update; on failure: log warning and continue`.
...
**Goal:** Add a conditional graphify preference instruction to `agents/super-spec-planner.md` and `agents/super-spec-pattern-mapper.md`. The instruction must: (a) check for `graphify-out/wiki/index.md` existence; (b) if present, prefer `graphify query/path/explain` over reading flat ARCH.md and TECH.md; (c) keep QUALITY.md, CONCERNS.md, DOMAIN.md reads unchanged.

**Files:**
- `agents/super-spec-planner.md`
- `agents/super-spec-pattern-mapper.md`

**blockedBy:** (none)

**read_first:**
- `agents/super-spec-planner.md` (full; already read - the role boundary section)
- `agents/super-spec-pattern-mapper.md` (full; already read)
- `PATTERNS.md` concept: "agent prompt instruction update (graphify query preference)"

**Verify:** `grep -n 'graphify.*query\|graphify.*path\|graphify.*explain\|graphify-out/wiki' agents/super-spec-planner.md` exits 0. `grep -n 'graphify.*query\|graphify.*path\|graphify.*explain\|graphify-out/wiki' agents/super-spec-pattern-mapper.md` exits 0.

**Acceptance criteria:**
- [ ] `grep -n 'graphify.*query\|graphify.*path\|graphify.*explain\|graphify-out/wiki' agents/super-spec-planner.md` exits 0.
- [ ] `grep -n 'graphify.*query\|graphify.*path\|graphify.*explain\|graphify-out/wiki' agents/super-spec-pattern-mapper.md` exits 0.
...
- [ ] Step 2: In `agents/super-spec-planner.md`, add a "Graphify-first navigation" block to the Role boundary or Procedure section: "If `graphify-out/wiki/index.md` exists, prefer `graphify query "<question>"`, `graphify path "<A>" "<B>"`, `graphify explain "<concept>"` for structural and architectural questions over reading flat ARCH.md or TECH.md. QUALITY.md, CONCERNS.md, and DOMAIN.md reads are unchanged."
- [ ] Step 3: Apply the same instruction block to `agents/super-spec-pattern-mapper.md` in the appropriate section (Step 1 - Read inputs or Find analogs).
...
2. For task-005 (agent deletion): restore from `git show HEAD~N:agents/super-spec-mapper-arch.md` if needed.
