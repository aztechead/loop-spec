...
The current super-spec cycle dispatches every subagent through a one-shot `Agent({subagent_type, model, prompt})` call. The subagent runs to completion, returns a single report, and dies. This shape forces three structural compromises:
...
- Preserve the user-facing cycle contract: same tier / preset / style / title prompts, same artifact paths under `docs/super-spec/features/{slug}/`, same exit semantics for AUTO / STEP / INTERACTIVE / REVIEW-ONLY.
...
1. **Startup health-check gains a team-capability probe.** Step 2 still probes the 3 model IDs and the 1M-context flag, then additionally:
   - Checks that `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` is set. If unset, abort with:
     ...
     ```
   - Runs a **harness API capability probe** that creates a throwaway team `super-spec-probe-{pid}` with one teammate, then exercises the three contracts the cycle hard-requires:
...
3. **Resume behavior is more restrictive.** Instead of scanning `.super-spec/features/*/state.json`, the cycle scans `.super-spec/features/*/feature.json` (a thin manifest in the gitignored state directory, see "State migration" below). Resume is only offered for features whose phase has a clean restart point (a phase whose team is gone, or a phase with no live teammates). In-process teammate resume is explicitly not supported, and the cycle prints a one-line note when it skips a stale in-progress feature for that reason.

All artifact paths (`docs/super-spec/features/{slug}/SPEC.md`, `PLAN.md`, etc.) and all user prompts (tier / preset / style / title) are unchanged.
...
| Gate history | Appended to `docs/super-spec/features/{slug}/feature.json` (lightweight log, see schema below) |
...
- **Team name:** `super-spec-discuss-{slug}`
...
- **Team name:** `super-spec-plan-{slug}`
...
- **Team name:** `super-spec-execute-{slug}`
- **Lead:** cycle main thread
- **Teammates:**
...
- **Communication pattern:**
  ...
  - **Exclusion list.** By default, all file overlaps are flagged (the default exclusion list is empty). Projects configure exclusions via either:
    ...
    - `.super-spec/file-conflict-exclude.txt` (one glob per line, repo-wide, lives in the gitignored state dir).
    Both sources are unioned. The spec ships **no project-specific defaults** so super-spec stays generic across consuming repos.
...
- **Team name:** `super-spec-verify-{slug}`
- **Lead:** cycle main thread
- **Teammates:**
...
- **Team name:** `super-spec-map-codebase-{project-id}` (project-id derived from repo root path hash)
...
- **Hooks:** `TaskCompleted` validates each `docs/super-spec/codebase/{DOMAIN}.md` against schema before accepting.
...
| Phase / artifact paths / tier / preset / style / commands / branch / baseSha | `state.json` top-level fields | `.super-spec/features/{slug}/feature.json` (new lightweight manifest in the gitignored state dir, see schema below) |
| Gate history | `state.json` `gateHistory[]` | `.super-spec/features/{slug}/feature.json` `gateHistory[]` (same shape) |
| Per-phase retry budget | `state.json` `retryBudget.perPhaseUsed` | `.super-spec/features/{slug}/feature.json` `retryBudget` (same shape, minus `executePerTask` which moves to per-task metadata) |
| Per-task retry counter | `state.json` `tasks[].retries` | Harness task metadata `retries` field |
| Codebase-source provenance | `state.json` `artifacts.codebaseSource` | `.super-spec/features/{slug}/feature.json` `artifacts.codebaseSource` (same shape) |
| Warnings | `state.json` `warnings[]` | `.super-spec/features/{slug}/feature.json` `warnings[]` |

**Path split rationale.** Committed artifacts (`SPEC.md`, `PLAN.md`, `PATTERNS.md`, `EXECUTION.md`, `VERIFICATION.md`) remain under `docs/super-spec/features/{slug}/` exactly as today. Volatile state (`updatedAt`, `currentTeamName`, `currentTeammates`, `currentGate`, `retryBudget.*Used`, `warnings`) lives in `.super-spec/features/{slug}/feature.json`, which is gitignored. This keeps churn out of the tracked tree while preserving the discoverability of artifacts.

### `feature.json` schema (replaces `state.json`)

Path: `.super-spec/features/{slug}/feature.json` (gitignored).
...
- Atomic write via a new `lib/feature-write.sh` (same temp-rename pattern as `state-write.sh`, but writing to `.super-spec/features/{slug}/feature.json` instead). Backup file is `feature.json.bak`.
- `.gitignore` gains a `.super-spec/` entry as part of this change.
...
| `state.json` removed | Files in `.super-spec/features/{slug}/state.json` are no longer read or written. | Any in-flight feature mid-cycle at upgrade time must be completed on the prior version (`git checkout v0.3.2`) or restarted from scratch. The cycle does not auto-migrate v2 state to v3. |
...
| State path moved | `feature.json` lives under `.super-spec/features/{slug}/` (gitignored), not `docs/super-spec/features/{slug}/`. | Anything that grepped `docs/super-spec/features/*/feature.json` for live state must read `.super-spec/features/*/feature.json`. Committed artifacts (`SPEC.md`, `PLAN.md`, `PATTERNS.md`, `EXECUTION.md`, `VERIFICATION.md`) stay in `docs/super-spec/features/{slug}/`. |
...
1. **Resume detection (Step 1)** scans `.super-spec/features/*/feature.json`.
...
- [ ] Running `Skill(super-spec:cycle)` with `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` unset aborts at Step 2 with the documented error message; running with it set proceeds past Step 2.
- [ ] Step 2 capability probe runs to completion against a real harness (creates `super-spec-probe-{pid}`, exercises `TaskUpdate` with `awaiting_review`, writes/reads task `metadata`, sends a `SendMessage` round-trip, calls `TeamDelete`). Verify by inspecting the run log for `[PROBE] PASSED` lines for each of the three checks. With the env var set on a supported harness, Step 2 returns success; with `metadata` write disabled or `awaiting_review` rejected on a doctored harness, Step 2 aborts with the documented error message.
- [ ] After a successful cycle run on the smoke fixture, `.super-spec/features/{slug}/feature.json` exists, validates against schema v3, has `currentPhase == "completed"` and `currentTeamName == null` and `currentTeammates == []` and `currentGate.round == 0`, and no `state.json` exists anywhere under `.super-spec/features/{slug}/`. The committed `docs/super-spec/features/{slug}/` directory contains `SPEC.md`, `PLAN.md`, `PATTERNS.md`, `EXECUTION.md`, `VERIFICATION.md` and **no `feature.json`**.
- [ ] `.gitignore` contains a `.super-spec/` entry; `git status --ignored` confirms `.super-spec/features/{slug}/feature.json` is ignored.
