...
**Decision:** PROCEED via WORKAROUND. Smoke runner does NOT pipe stdin to interactive prompts. Instead, cycle skill supports env-var overrides: `SUPER_SPEC_NON_INTERACTIVE=1`, `SUPER_SPEC_ANSWER_TIER=quick`, `SUPER_SPEC_ANSWER_STYLE=auto`, `SUPER_SPEC_ANSWER_TITLE="..."`. Cycle skill detects env var and skips AskUserQuestion calls, reading answers from env. Standard pattern; doesn't require any CC harness probe.
