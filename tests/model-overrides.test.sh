...
check "default: every role but the reader roles and the route judge inherits" \
  "$(echo "$models" | jq -e '[del(.challenger, .codeReviewer, .specComplianceReviewer, .routeJudge)[]] | all(. == "inherit")' >/dev/null 2>&1 && echo 1 || echo 0)"
...
check "default: code-reviewer, spec-compliance-reviewer are sonnet on Claude Code" \
  "$(echo "$default_out" | jq -e '.codeReviewer == "sonnet" and .specComplianceReviewer == "sonnet"' >/dev/null 2>&1 && echo 1 || echo 0)"
check "default: code-reviewer, spec-compliance-reviewer inherit on OpenCode (no alias surface)" \
  "$(echo "$oc_default" | jq -e '.codeReviewer == "inherit" and .specComplianceReviewer == "inherit"' >/dev/null 2>&1 && echo 1 || echo 0)"
