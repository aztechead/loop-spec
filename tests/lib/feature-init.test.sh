...
check "single perGate==3" "$(echo "$single" | jq -e '.retryBudget.perGate == 3' >/dev/null 2>&1 && echo 1 || echo 0)"
check "single iterate.maxIterations==10" "$(echo "$single" | jq -e '.iterate.maxIterations == 10' >/dev/null 2>&1 && echo 1 || echo 0)"
check "single global budget==40" "$(echo "$single" | jq -e '.retryBudget.global == 40' >/dev/null 2>&1 && echo 1 || echo 0)"
check "single perPhase.iterate==10" "$(echo "$single" | jq -e '.retryBudget.perPhase.iterate == 10' >/dev/null 2>&1 && echo 1 || echo 0)"
