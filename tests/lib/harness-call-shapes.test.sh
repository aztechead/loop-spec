...
# (skills/shared/dispatch.md). A call that "reads right" but fails the
...
bad=$(grep -rn 'options: \["' skills agents --include='*.md' 2>/dev/null | grep -v 'shared/dispatch.md' | head -5 || true)
...
grep -qF 'one-shot mode returns the final report directly' skills/shared/team-prompts/critic.md \
  && grep -qF 'Team mode sends the report with `SendMessage({to: "team-lead", message, summary})`' \
    skills/shared/team-prompts/critic.md \
  && v=1 || v=0
check "critic report transport stays neutral across adapters" "$v"
grep -qF 'Read the critique assignment at <promptFile> and complete it.' \
  skills/shared/critique-gate-protocol.md && v=1 || v=0
check "critique coordinator dispatches the prompt file by reference" "$v"
...
bad=$(grep -rnE 'model: "?claude-' skills agents --include='*.md' 2>/dev/null | grep -v 'shared/dispatch.md\|model-matrix' | head -5 || true)
...
check "dispatch.md present" "$([[ -f skills/shared/dispatch.md ]] && echo 1 || echo 0)"
grep -q 'Verification method' skills/shared/dispatch.md && v=1 || v=0
...
grep -qF 'in-process teammate' skills/shared/dispatch.md \
  && grep -qF 'lib/implicit-team-model.sh' skills/shared/dispatch.md \
  && v=1 || v=0
...
grep -qF 'not a real question' skills/shared/dispatch.md && v=1 || v=0
...
grep -qF 'placeholder-question-guard.sh' skills/shared/dispatch.md \
  && v=1 || v=0
