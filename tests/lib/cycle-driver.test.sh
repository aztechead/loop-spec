...
# The .gitignore exceptions land as one delimited block, never under a foreign comment.
check "next: gitignore block carries the loop-spec header" "1" "$(grep -c '^# loop-spec: keep these tracked' "$REPO/.gitignore")"
