...
check "reviewable estimate wins" "budgetReason=route-size-estimate:3-files-2-criteria" "$(bash "$DB" --feature-dir "$FD" --phase plan | awk '{print $NF}')"
...
check "duplicate and invalid telemetry" "elapsed=2" "$(bash "$DB" --feature-dir "$FD" --phase spec | cut -d' ' -f2)"
...
check "initial design pass exhausts" "elapsed=60" "$(bash "$DB" --feature-dir "$FD" --phase plan | cut -d' ' -f2)"
printf '%s\n' \
