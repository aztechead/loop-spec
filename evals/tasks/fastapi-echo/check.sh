#!/usr/bin/env bash
# Acceptance for fastapi-echo. Prints one `CHECK <name> PASS|FAIL <note>` line per criterion.
...
c = TestClient(app)
r = c.get("/echo", params={"text": "hello there"})
assert r.status_code == 200, r.status_code
assert r.json() == {"text": "hello there"}, r.json()
r = c.post("/echo", json={"text": "round trip"})
assert r.status_code in (200, 201), r.status_code
assert r.json() == {"text": "round trip"}, r.json()
r = c.post("/echo", json={"text": 42})
assert r.status_code == 422, r.status_code
r = c.post("/echo", json={"text": ["a"]})
assert r.status_code == 422, r.status_code
print("ok")
PY
)"
if [[ "$out" == *ok ]]; then echo "CHECK get_and_post_echo PASS"; else echo "CHECK get_and_post_echo FAIL $(tail -1 <<<"$out")"; fi
